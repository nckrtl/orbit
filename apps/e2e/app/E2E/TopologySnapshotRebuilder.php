...
{
    /** The copy a promotion makes before it swaps a topology snapshot instance into place. */
     ...
     * topology snapshot resource is absent, so a cold build can construct it again.
     *
    ...
    public function teardown(): array
    {
        ...
        try {
            $present = array_keys($this->host->instances($this->topologySnapshotInstanceNames()));
            ...
            if ($present !== []) {
                ...
                throw new RuntimeException(
                    'Topology snapshot resources are present: '
                    .implode(', ', $present)
                    .'. Use bin/e2e-topology-snapshot recover-legacy --main-sha=<sha>.',
                );
            }
    ...
    public function recover(
        LegacyTopologySnapshotInventory $authorization,
        Closure $record,
        ...
        if ($network !== null && $authorizedNetwork !== $network->name) {
            throw new RuntimeException('The exact topology snapshot network inventory changed after authorization.');
        }
        ...
        if ($promoted !== null && $promoted->toArray() !== $authorization->promotedManifest) {
            throw new RuntimeException('The promoted topology snapshot manifest changed after authorization.');
        }
        foreach ($this->manifests->recorded() as $recorded) {
            if (! in_array($recorded->toArray(), $authorization->recordedManifests, true)) {
                throw new RuntimeException('A recorded topology snapshot manifest changed after authorization.');
            }
        }
    ...
    private function deleteInstances(bool $allowPromotionCopy = false): array
    {
        ...
        if ($this->host->instances($names) !== []) {
            throw new RuntimeException('A topology snapshot VM persisted after deletion.');
        }
    ...
    private function deleteNetwork(): array
    {
        ...
        // metadata; the harness prefix and the topology snapshot name are the identity.
        ...
        if ($this->host->network($name) !== null) {
            throw new RuntimeException('The topology snapshot network persisted after deletion.');
        }
    ...
    {
        $this->manifests->forgetAll();
        if ($this->manifests->promoted() !== null) {
            throw new RuntimeException('The promoted topology snapshot manifest persisted after the teardown.');
        }
    }

    /** @return list<string> */
    private function topologySnapshotInstanceNames(): array
    {
        $names = [];
    ...
    private function assertHarnessOwned(
        ...
        if (($instance->metadata['user.orbit.e2e.owner'] ?? null) !== 'orbit-e2e') {
            throw new RuntimeException(
                "Incus instance {$name} is not harness-owned; the topology snapshot rebuild refuses to delete it.",
            );
        }
        ...
        if (is_string($issue) && $issue !== '') {
            ...
            throw new RuntimeException(
                "Incus instance {$name} belongs to issue {$issue}; release that topology before rebuilding the topology snapshot.",
            );
        }
    }
    ...
    private function assertInventoryUnchanged(
        ...
        foreach ($present as $name => $instance) {
            ...
            if (! is_array($authorized)) {
                throw new RuntimeException(
                    'The exact topology snapshot instance inventory changed after authorization.',
                );
            }
        ...
        foreach ($snapshots as $name => $current) {
            if (($authorization->snapshots[$name] ?? null) !== $current) {
                throw new RuntimeException('The exact Incus snapshot inventory changed after authorization.');
            }
        }
        ...
        if ($network !== null) {
            ...
            if ($authorization->network === null) {
                throw new RuntimeException(
                    'The exact topology snapshot network inventory changed after authorization.',
                );
            }
