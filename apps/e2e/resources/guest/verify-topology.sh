...
case "$probe" in
  ...
  laravel.prod) [[ -f /var/www/laravel/e2e-prod/artisan ]] && php /var/www/laravel/e2e-prod/artisan --version >/dev/null && curl --fail --silent --show-error --max-time 5 --cacert "$(cat /var/lib/orbit-e2e/caddy-ca-path)" https://laravel.internal/ >/dev/null; expected='app-prod-laravel:https-operational'; observed=$expected ;;
