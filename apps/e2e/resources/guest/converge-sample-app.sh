...
case ${1-} in
  ...
  hydrate)
    ...
    if [[ "$3" == app-prod ]]; then
      ...
      [[ -s "$ca" ]]
      if [[ $# -eq 4 ]]; then
        curl --fail --silent --show-error --retry 10 --retry-delay 2 --retry-connrefused --retry-all-errors --connect-timeout 10 --max-time 30 --cacert "$ca" --resolve "$production_domain:443:127.0.0.1" "https://$production_domain/" >/dev/null
      else
