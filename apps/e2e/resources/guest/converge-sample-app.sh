...
case ${1-} in
  ...
  create-resources)
    ...
    if [[ "$instance_shape" == instances ]]; then
      ...
      existing_project=$(php -r '$v=json_decode(stream_get_contents(STDIN), true, 512, JSON_THROW_ON_ERROR); $items=$v["projects"] ?? $v["apps"] ?? null; if(!is_array($items) || !array_is_list($items)) exit(65); $m=array_values(array_filter($items, fn($x) => ($x["slug"] ?? null)===$argv[1])); if(count($m)>1) exit(65); if($m) { $x=$m[0]; if(($x["repository_url"] ?? null)!==$argv[2] || ($x["name"] ?? null)!==$argv[3] || ($x["root"] ?? null)!==$argv[4] || ($argv[5]==="project" && ($x["type"] ?? null)!=="laravel-app")) exit(65); echo json_encode($x, JSON_THROW_ON_ERROR); }' laravel-typed https://github.com/laravel/laravel.git Laravel public "$project_command" <<<"$apps")
      app_id=
      app_branch=
