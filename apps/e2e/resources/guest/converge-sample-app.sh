...
case ${1-} in
  ...
  create-resources)
    ...
    if [[ "$instance_shape" == app_instances ]]; then
      ...
      # The cluster router stays on app-dev, but the one sample AppInstance is
      # placed on the original app-prod Node. The extra app-prod Node remains empty.
      ...
      if [[ "$typed_count" -eq 0 ]]; then
        "$orbit" instance:new "$app_id" "$sample_node_id" e2e-dev --hostname=e2e-dev.orbit --json >/dev/null
