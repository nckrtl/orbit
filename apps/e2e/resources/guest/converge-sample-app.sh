  ...
  create-resources)
    ...
    if [[ "$instance_shape" == app_instances ]]; then
      ...
      typed_production_state() {
        php -r '$v=json_decode(stream_get_contents(STDIN), true, 512, JSON_THROW_ON_ERROR); if(!is_array($v) || !is_array($v["app_instances"] ?? null) || !array_is_list($v["app_instances"])) exit(65); $m=array_values(array_filter($v["app_instances"], fn($x) => is_array($x) && ($x["name"] ?? null)===$argv[1])); if(count($m)!==1) exit(65); $x=$m[0]; $id=$x["id"] ?? null; $home=$x["production_home"] ?? null; $user=$x["production_user"] ?? null; $checkout=$x["checkout_path"] ?? null; $root=$x["effective_root"] ?? null; $ok=static fn(mixed $v): bool => is_string($v) && preg_match("/\\A[a-z0-9](?:[a-z0-9.-]{0,251}[a-z0-9])?\\z/D", $v)===1; $hostname=null; if(array_key_exists("domain", $x)) { if(!$ok($x["domain"])) exit(65); $hostname=$x["domain"]; } elseif(array_key_exists("hostname", $x)) { if(!$ok($x["hostname"])) exit(65); $hostname=$x["hostname"]; } else { $nested=is_array($x["route"] ?? null) && !array_is_list($x["route"]) ? $x["route"] : []; if(array_key_exists("domain", $nested)) { if(!$ok($nested["domain"])) exit(65); $hostname=$nested["domain"]; } elseif(array_key_exists("hostname", $nested)) { if(!$ok($nested["hostname"])) exit(65); $hostname=$nested["hostname"]; } else exit(65); } $layout=(($x["source_layout"] ?? null)==="release" || ($x["source_layout"] ?? null)==="releases" || is_string($checkout) && str_ends_with($checkout, "/current")) ? "release" : "flat"; $path=static fn(mixed $p): bool => is_string($p) && str_starts_with($p, "/") && !str_contains($p, "//") && preg_match("#(?:\\A|/)\\.\\.?(/|\\z)#D", $p)!==1; if(!is_int($id) || $id<1 || ($x["app_id"] ?? null)!==(int)$argv[2] || ($x["node_id"] ?? null)!==(int)$argv[3] || ($x["environment"] ?? null)!=="production" || ($x["status"] ?? null)!=="active" || !is_string($user) || preg_match("/\\A[a-z_][a-z0-9_-]{0,31}\\z/D", $user)!==1 || !$path($home) || !$path($checkout) || !$path($root) || !is_string($hostname) || $hostname==="") exit(65); $version=is_string($x["php_version"] ?? null) ? $x["php_version"] : "8.5"; if(preg_match("/\\A[0-9]+\\.[0-9]+\\z/D", $version)!==1) exit(65); $release=$layout==="release" ? ($x["current_target"] ?? null) : null; if($layout==="release" && (!$path($release) || !str_starts_with($release, $home."/releases/"))) exit(65); $service=$layout==="release" ? "orbit-".$user."-php".$version."-fpm.service" : "php".$version."-fpm.service"; $socket=$layout==="release" ? "/run/php/".$user.".sock" : "/run/php/orbit-prod-instance-".$id.".sock"; echo json_encode(["layout"=>$layout,"instance_id"=>$id,"user"=>$user,"home"=>$home,"checkout_path"=>$checkout,"effective_root"=>$root,"environment_path"=>$home."/.env","database_path"=>null,"service"=>$service,"socket"=>$socket,"current_target"=>$release,"hostname"=>$hostname], JSON_THROW_ON_ERROR);' e2e-prod "$app_id" "$prod_id"
      }
      typed_route_id() {
        php -r '$v=json_decode(stream_get_contents(STDIN), true, 512, JSON_THROW_ON_ERROR); $list=$argv[4]==="list"; if($list) { if(!is_array($v) || array_is_list($v) || !is_array($v["routes"] ?? null) || !array_is_list($v["routes"]) || !is_string($v["request_id"] ?? null) || $v["request_id"]==="") exit(65); $routes=$v["routes"]; } else { if(!is_array($v) || array_is_list($v) || !is_string($v["request_id"] ?? null) || $v["request_id"]==="") exit(65); $routes=[$v]; } $matches=[]; foreach($routes as $route) { if(!is_array($route) || array_is_list($route)) exit(65); foreach(["id","app_id","node_id","cluster_id","generation_basis_node_id","provenance","publication","status","failed_step","error_code","target"] as $key) if(!array_key_exists($key, $route)) exit(65); $ok=static fn(mixed $v): bool => is_string($v) && preg_match("/\\A[a-z0-9](?:[a-z0-9.-]{0,251}[a-z0-9])?\\z/D", $v)===1; if(array_key_exists("domain", $route)) { if(!$ok($route["domain"])) exit(65); $endpoint=$route["domain"]; } elseif(array_key_exists("hostname", $route)) { if(!$ok($route["hostname"])) exit(65); $endpoint=$route["hostname"]; } else exit(65); $target=$route["target"]; if(!is_int($route["id"]) || $route["id"]<1 || !is_int($route["app_id"]) || $route["app_id"]<1 || (!is_int($route["node_id"]) && $route["node_id"]!==null) || (!is_int($route["cluster_id"]) && $route["cluster_id"]!==null) || (($route["node_id"]===null)===($route["cluster_id"]===null)) || (!is_int($route["generation_basis_node_id"]) && $route["generation_basis_node_id"]!==null) || !in_array($route["provenance"], ["generated","explicit"], true) || !in_array($route["publication"], ["private","public"], true) || !is_string($route["status"]) || (!is_string($route["failed_step"]) && $route["failed_step"]!==null) || (!is_string($route["error_code"]) && $route["error_code"]!==null)) exit(65); if($target!==null && (!is_array($target) || array_is_list($target) || !is_int($target["id"] ?? null) || $target["id"]<1 || !is_int($target["app_instance_id"] ?? null) || $target["app_instance_id"]<1 || !is_int($target["position"] ?? null) || $target["position"]<0)) exit(65); $targetsInstance=is_array($target) && $target["app_instance_id"]===(int)$argv[2]; if($endpoint==="e2e-dev.orbit" || $targetsInstance) $matches[]=$route; } if($list && $matches===[]) { echo "missing"; exit; } if(count($matches)!==1) exit(65); $route=$matches[0]; if(array_key_exists("domain", $route)) { if(!$ok($route["domain"])) exit(65); $endpoint=$route["domain"]; } elseif(array_key_exists("hostname", $route)) { if(!$ok($route["hostname"])) exit(65); $endpoint=$route["hostname"]; } else exit(65); $target=$route["target"]; if($route["app_id"]!==(int)$argv[1] || $route["node_id"]!==null || $route["cluster_id"]!==(int)$argv[3] || $route["generation_basis_node_id"]!==null || $endpoint!=="e2e-dev.orbit" || $route["provenance"]!=="explicit" || $route["publication"]!=="private" || !is_array($target) || $target["app_instance_id"]!==(int)$argv[2] || $target["position"]!==0) exit(65); echo $route["id"];' "$app_id" "$1" "$cluster_id" "$2"
      }
      ...
      if [[ "$typed_count" -eq 0 ]]; then
        instance_create_help=$("$orbit" instance:create --help)
        if grep -Eq -- '(^|[[:space:]])--domain(=|\[=|[[:space:]]|$)' <<<"$instance_create_help"; then
          endpoint_flag=domain
        else
          endpoint_flag=hostname
        fi
        "$orbit" instance:create "$app_id" "$dev_id" e2e-dev --${endpoint_flag}=e2e-dev.orbit --json >/dev/null
