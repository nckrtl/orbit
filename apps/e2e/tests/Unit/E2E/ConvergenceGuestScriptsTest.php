...
/** @return array{root:string,script:string,state:string,legacy_records:array<string,string>} */
function typed_sample_resource_fixture(): array
{
    $root = temporaryPath('orbit-typed-sample-resources-', 5);
    mkdir($root, 0o700, true);
    $legacyRecords = [
        'app' => "{$root}/legacy-app.json",
        'instance' => "{$root}/legacy-instance.json",
        'workspace' => "{$root}/legacy-workspace.json",
    ];
    file_put_contents(
        $legacyRecords['app'],
    ...
    file_put_contents("{$root}/orbit", <<<'BASH'
        ...
        route_json() {
          ...
          case "${ROUTE_ENDPOINT_SHAPE:-domain}" in
            domain) ;;
            hostname) endpoint='"hostname":"e2e-dev.orbit"' ;;
        ...
        case "$1" in
          ...
          app:list|project:list)
            ...
            if [[ -n "${TYPED_APP_RESPONSE:-}" ]]; then
              ...
              printf '{"%s":[%s,{"id":1,"slug":"laravel-typed","type":"laravel-app","name":"Laravel","repository_url":"https://github.com/laravel/laravel.git","main_branch":"main","root":"public"}]}' "$collection" "$legacy"
            else
          ...
          instance:list)
            ...
            if [[ ! -e "$state/instance" && -n "${INITIAL_TYPED_RESPONSE:-}" ]]; then
              printf '%s' "$INITIAL_TYPED_RESPONSE"
            elif [[ -e "$state/instance" ]]; then
              if [[ -n "${TYPED_RESPONSE:-}" ]]; then
                ...
                if [[ -e "$state/production" ]]; then
                  ...
                  case "${PRODUCTION_ENDPOINT_SHAPE:-domain}" in
                    domain) ;;
                    hostname) production_endpoint='"hostname":"e2e-prod.orbit.test"' ;;
    ...
    it('fails closed for production service socket owner and current-target mismatches', function (): void {
        ...
        try {
            expect($run('role.app-prod', $placement)->run())
                ...
                ->toBe(0);
            $hostnamePlacement = array_replace(
                array_diff_key($placement, ['domain' => true]),
                ['hostname' => $placement['domain']],
            );
            expect($run('role.app-prod', $hostnamePlacement)->run())
                ->not
                ->toBe(0)
                ->and($run('php-fpm.app-prod', $hostnamePlacement)->run())
                ->not
                ->toBe(0);
...
describe('convergence guest scripts', function () {
    ...
    it('uses legacy App commands without creating duplicate workspace sample resources', function () {
    ...
    it('requires the Laravel application type when reusing a current sample Project', function (?string $type, int $status): void {
        ...
        $response = json_decode(typed_sample_app($project), true, flags: JSON_THROW_ON_ERROR);
        if ($type === null) {
            unset($response['type']);
        }
        ...
        try {
            $process = typed_sample_create_resources_process($fixture, [
                ...
                'TYPED_APP_RESPONSE' => json_encode($response, JSON_THROW_ON_ERROR),
            ]);
    ...
    it('invokes only domain-form instance:create input', function (): void {
        $fixture = typed_sample_resource_fixture();
        try {
            $process = typed_sample_create_resources_process($fixture);
            ...
            expect($commands)
                ...
                ->not->toContain('instance:create --help')
    ...
    it('accepts a valid application domain on Routes and production AppInstances', function (): void {
        ...
        try {
            ...
            expect($process->run())->toBe(0, $process->getErrorOutput());
            $state = json_decode((string) file_get_contents($fixture['state']), true, 16, JSON_THROW_ON_ERROR);
            expect($state['production']['domain'] ?? null)->toBe('e2e-prod.orbit.test');
            expect($state['production'])->not->toHaveKey('hostname');
    ...
    it('refuses hostname-only Route and production responses without fallback', function (): void {
        $fixture = typed_sample_resource_fixture();
        $surface = implode("\n", ['instance:clone', 'instance:deploy', 'instance:deploy-step:create'])."\n";
        try {
            $process = typed_sample_create_resources_process($fixture, [
    ...
    it('refuses a present invalid domain without falling back to hostname', function (
        ...
        'invalid Route domain' => ['invalid-domain-with-hostname', 'domain'],
        'invalid production domain' => ['domain', 'invalid-domain-with-hostname'],
        'hostname-only Route' => ['hostname', 'domain'],
        'hostname-only production' => ['domain', 'hostname'],
    ]);
    ...
    it('recovers missing development source metadata before the first production clone', function (string $error, bool $recovers, bool $succeeds): void {
        ...
        try {
            ...
            $clone = array_search('instance:clone 4 3 e2e-prod --branch=main --preview-name=e2e-prod --json', $commands, true);
    ...
    it('inspects a replacement Route while selecting the authoritative private domain', function (): void {
        $fixture = typed_sample_resource_fixture();
        foreach (['cluster', 'attached', 'router', 'active', 'app', 'instance'] as $stateFile) {
            touch("{$fixture['root']}/{$stateFile}");
        }
        ...
        try {
            $process = typed_sample_create_resources_process($fixture, [
                'ROUTE_LIST_RESPONSE' => typed_sample_route_list([
                    typed_sample_route([
                        'id' => 8,
                        'domain' => 'e2e-dev-next.orbit',
                        'status' => 'activating',
                        'replaces_route_id' => 5,
                        'replacement_step' => 'activate',
                    ]),
                    typed_sample_route([
                        'replaced_by_route_id' => 8,
                        'replacement_step' => 'retire',
                    ]),
                ]),
            ]);
    ...
    it('refuses conflicting or malformed typed Routes without Route mutation', function (string $response): void {
        ...
        'hostname without domain' => [typed_sample_route_list([typed_sample_route(['hostname' => 'e2e-dev.orbit'])])],
        ...
        'missing replacement fields' => [typed_sample_route_list([array_diff_key(
            typed_sample_route(),
            ['replaces_route_id' => true, 'replaced_by_route_id' => true, 'replacement_step' => true],
        )])],
        'multiple associations' => [typed_sample_route_list([
            typed_sample_route(),
            typed_sample_route(['id' => 7, 'domain' => 'second.orbit']),
        ])],
    ]);
    ...
    it('rejects typed sample conflicts before App or AppInstance mutation', function (string $conflict): void {
        ...
        $environment = match ($conflict) {
            'App' => [
                'TYPED_APP_RESPONSE' => json_encode([
                    'id' => 1,
                    'slug' => 'laravel-typed',
                    'name' => 'Laravel',
    ...
    it('fails closed when created typed source state is not authoritative', function (string $response): void {
        ...
        try {
            $process = new Process([
                ...
                'app-prod',
                str_repeat('a', 40),
            ], env: ['TYPED_RESPONSE' => str_replace(
                '{checkout}',
                $fixture['root'].'/laravel-typed/e2e-dev',
                $response,
            )]);

            expect($process->run())->not->toBe(0);
            $commands = file("{$fixture['root']}/commands", FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
            expect($commands)->toContain('instance:create 1 2 e2e-dev --domain=e2e-dev.orbit --json');
            expect(file_exists($fixture['state']))->toBeFalse();
            expect(implode("\n", $commands))
                ->not
                ->toContain(
