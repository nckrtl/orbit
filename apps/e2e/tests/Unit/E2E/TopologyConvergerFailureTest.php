...
describe('TopologyConverger failures', function () {
    ...
    it('ignores arbitrary output and malformed gateway diagnostics', function (string $output): void {
        Process::fake(function (PendingProcess $process) use ($output) {
            ...
            if (in_array('/usr/local/bin/converge-gateway.sh', $command, true)) {
                return Process::result($output, '', 71);
            }
        ...
        try {
            ...
            expect($exception->getMessage())
                ->toBe(
                    'Guest convergence script converge-gateway.sh failed on orbit-e2e-nck-123-gateway with exit code 71.',
                )
                ->not->toContain('sensitive', 'wireguard-server-install');
        }
    })->with([
        'stale marker' => ["orbit-e2e-failure step=old error=old\n".'sensitive'],
        'malformed stable line' => [
            "Gateway bootstrap failed at step [wireguard-server-install] with error [vpn.server_config_install_failed]\n"
                .'sensitive',
        ],
    ]);
