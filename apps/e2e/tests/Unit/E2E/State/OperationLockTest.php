...
describe('OperationLock', function () {
    it('allows shared generation pins and bounds an exclusive waiter', function () {
        ...
        expect($first->acquire('generation-g1', new OperationId('operation-one'), false, 0.05))
            ->toBeTrue()
            ->and($second->acquire('generation-g1', new OperationId('operation-two'), false, 0.05))
            ->toBeTrue();

        $started = microtime(true);
        expect($refresh->acquire('generation-g1', new OperationId('refresh-one'), true, 0.05))
            ->toBeFalse()
    ...
    it('persists caller ownership and verifies stale process identity', function () {
        ...
        $operation = new OperationId('operation-owner');
    ...
    it('uses an injected process identity when proc is unavailable', function () {
        ...
        expect($lock->acquire('portable', new OperationId('portable-owner'), true, 0.05))->toBeTrue();
        $owner = json_decode(
    ...
    it('does not leak a lock when process identity resolution fails', function () {
        ...
        expect(fn () => $failing->acquire('recoverable', new OperationId('resolver-fails'), true, 0.05))
            ->toThrow(RuntimeException::class, 'process start');

        $next = new OperationLock($paths, fn (int $pid): string => 'start-'.$pid);
        expect($next->acquire('recoverable', new OperationId('resolver-retry'), true, 0.05))->toBeTrue();
        $next->release();
    });
    ...
    it('closes stale-cleanup handles when another operation holds the lock', function () {
        ...
        $holder->acquire('contended', new OperationId('holder-owner'), true, 0.05);
        $staleOwner = [
            'pid' => 999_999_999,
            'process_start_identity' => '1',
            'operation_id' => 'stale-contender',
            'acquired_at' => '2026-08-28T00:00:00Z',
        ];
