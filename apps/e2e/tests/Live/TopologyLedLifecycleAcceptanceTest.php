 ...
 * stays alive until release, and the same commit proves again. Every phase
...
it('proves the simple flow through public wrappers', function (): void {
    ...
    try {
        ...
        $secondRelease = LiveHarness::jsonPhase(
            'release second proof',
            fn (): array => LiveHarness::jsonWrapper('topology', 'release', $issue, $worktreeOption),
            $timings,
        );
        $releasedAttempts[] = $secondProofAttempt;
        lifecycleAssertRelease($secondRelease, $secondProof, $stateRoot, $worktreeOption);
