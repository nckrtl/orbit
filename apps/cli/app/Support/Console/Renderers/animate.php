 ...
 * The parent closes stdin to stop and joins this exact process before writing again.
 */
...
try {
    ...
    while (true) {
        $read = [STDIN];
        $write = $except = [];
        $remaining = max(1000, (int) ((($phase + 1) * 0.3 - (microtime(true) - $epoch)) * 1000000));
        ...
        $nextPhase = (int) floor(max(0, microtime(true) - $epoch) / 0.3);
