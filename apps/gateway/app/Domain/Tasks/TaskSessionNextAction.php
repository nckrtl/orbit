...
enum TaskSessionNextAction: string
{
    ...
    public static function choiceCriteria(): array
    {
        return [
            ...
            self::ContinueImplementer->value => 'The implementer is idle, done, or failed, done, or failed with unfinished work on the current brief.',
            self::RelayReviewToImplementer->value => 'The reviewer left a summary and the implementer is idle, done, or failed.',
            self::MarkSubtaskDone->value => 'The current subtask has evidence of completion beyond the thread state, is verified and should settle or hand off to review.',
