...
final class NativeProcessAdmissionLock implements ProcessAdmissionLock
{
    /** @var array<int, int> */
    ...
    public function run(array $appInstanceIds, Closure $operation): mixed
    {
        ...
        try {
            foreach ($ids as $id) {
                ...
                if ($handle === false) {
                    throw new RuntimeException("Could not open Process admission lock [{$path}].");
                }

                $handles[$id] = $handle;

                if (! chmod(filename: $path, permissions: 0o600)) {
                    throw new RuntimeException("Could not protect Process admission lock [{$path}].");
                }
    ...
    {
        if ($this->directory === '') {
            throw new RuntimeException('The Process admission lock directory is not configured.');
        }
        ...
        if (
            ...
            throw new RuntimeException("Could not create Process admission lock directory [{$this->directory}].");
        }

        if (! chmod(filename: $this->directory, permissions: 0o700)) {
            throw new RuntimeException("Could not protect Process admission lock directory [{$this->directory}].");
        }
    }
