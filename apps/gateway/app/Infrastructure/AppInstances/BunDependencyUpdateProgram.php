...
final class BunDependencyUpdateProgram
{
...
case "$vp" in /*) ;; *) exit 1 ;; esac
case "$vp" in *[[:cntrl:]]*) exit 1 ;; esac
[ -x "$vp" ] || exit 1
...
start=$(date +%s)
/usr/bin/setsid --wait "$vp" update --no-save -- --lockfile-only --save-text-lockfile </dev/null >/dev/null 2>&1 &
child=$!
...
' npm-update-watchdog "$owner" "$child" "$deadline" "$start" </dev/null >/dev/null 2>&1 &
watchdog=$!
while :; do
