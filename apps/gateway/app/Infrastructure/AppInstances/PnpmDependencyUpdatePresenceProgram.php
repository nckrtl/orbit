...
final class PnpmDependencyUpdatePresenceProgram
{
...
try:
    ...
    try:
        ...
        pnpm_lock = present(fd, 'pnpm-lock.yaml')
        pnpm_files = pnpm_lock or any(present(fd, name) for name in ('.pnpmfile.cjs', 'pnpmfile.cjs'))
        bun_files = any(present(fd, name) for name in ('bun.lock', 'bun.lockb', 'bunfig.toml'))
    ...
    if signals == ['pnpm']:
        status = 'present' if has_manifest and pnpm_lock else 'incomplete'
    elif not signals and has_manifest:
        status = 'incomplete'
    else:
        status = 'absent'
