...
final class TransferSourceProgram
{
    public static function definitions(): string
    {
        return <<<'PYTHON'
            ...
            class TransferSource:
                scope_name = ".orbit-transfer-sources"

                @staticmethod
                def require(condition):
                    if not condition:
                        raise ValueError("Transfer source ownership is unconfirmed.")
                ...
                @staticmethod
                def rename(source_parent, source, target_parent, target):
                    library = ctypes.CDLL(None, use_errno=True)
                    operation = library.renameat2
                    operation.argtypes = [ctypes.c_int, ctypes.c_char_p, ctypes.c_int, ctypes.c_char_p, ctypes.c_uint]
                    operation.restype = ctypes.c_int
                    if operation(source_parent, source.encode(), target_parent, target.encode(), 1) != 0:
                        raise OSError(ctypes.get_errno(), "Source claim was refused.")

                def __init__(self, specification):
                    self.specification = specification
                    self.attempt = specification["id"]
                    self.expected = specification["receipt"]
                    self.path = specification["source_path"]
                    ...
                    self.require(specification["private_root"].endswith("/.orbit/transfer-sources"))
                    self.require(not specification["private_root"].startswith(self.path.rstrip("/") + "/"))
                ...
                def binding(self):
                    return {
                        ...
                        "node_id": self.specification["node_id"], "source_path": self.path, "layout": self.specification["layout"],
                ...
                def read_state(self):
                    ...
                    self.require(state["phase"] in ("owned", "claiming", "claimed", "cleaned"))
                ...
                def check_receipt(self, receipt):
                    self.require(isinstance(receipt, dict) and set(receipt) == {"root", "parent", "scope", "checkout", "common_path", "common"})
                ...
                def not_ancestor(self, ancestor, path):
                    current = self.open_path(path)
                    try:
                        while True:
                            metadata = os.fstat(current)
                            self.require(self.identity(metadata) != ancestor)
                            parent = os.open("..", os.O_RDONLY | os.O_DIRECTORY | os.O_NOFOLLOW, dir_fd=current)
                            if self.identity(os.fstat(parent)) == self.identity(metadata):
                                os.close(parent)
                                return
                            os.close(current)
                            current = parent
                ...
                def git_path(self, checkout, argument):
                    result = subprocess.run(
                        ["git", "rev-parse", "--path-format=absolute", argument],
                        cwd="/proc/self/fd/" + str(checkout), pass_fds=(checkout,),
                        check=True, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL,
                    )
                    path = os.fsdecode(result.stdout.removesuffix(b"\n"))
                    self.require(path.startswith("/") and "\n" not in path)
                    return path

                def common(self, checkout, receipt=None):
                    metadata = self.metadata(checkout, ".git")
                    self.require(metadata is not None and metadata.st_uid == os.geteuid())
                    if self.specification["layout"] == "checkout":
                        self.require(stat.S_ISDIR(metadata.st_mode))
                        common_path, common = None, None
                    else:
                        self.require(stat.S_ISREG(metadata.st_mode) and metadata.st_nlink == 1)
                        common_path = self.git_path(checkout, "--git-common-dir")
                        descriptor = self.track(self.open_path(common_path))
                        self.owned(os.fstat(descriptor))
                        common = self.identity(os.fstat(descriptor))
                        self.not_ancestor(self.identity(os.fstat(checkout)), common_path)
                    if receipt is not None:
                        self.require(common_path == receipt["common_path"] and common == receipt["common"])
                    return common_path, common

                def observe(self):
                    self.require(self.specification["phase"] == "acquiring" and self.expected is None)
                    if self.state is not None:
                        self.require(self.state["phase"] == "owned")
                        self.expected = self.state["receipt"]
                        self.directory()
                        return self.expected
                    parent, scope = self.parent_and_scope(create=True)
                    self.require(self.name != self.scope_name and self.metadata(scope, self.claim_name) is None)
                    observed = self.metadata(parent, self.name)
                    self.owned(observed)
                    checkout = self.track(os.open(self.name, os.O_RDONLY | os.O_DIRECTORY | os.O_NOFOLLOW, dir_fd=parent))
                    self.require(self.identity(os.fstat(checkout)) == self.identity(observed))
                    top = self.track(self.open_path(self.git_path(checkout, "--show-toplevel")))
                    self.require(self.identity(os.fstat(top)) == self.identity(observed))
                    self.not_ancestor(self.identity(observed), self.specification["private_root"])
                    common_path, common = self.common(checkout)
                    receipt = {
                        "root": self.identity(os.fstat(self.root)), "parent": self.identity(os.fstat(parent)),
                        "scope": self.identity(os.fstat(scope)), "checkout": self.identity(observed),
                        "common_path": common_path, "common": common,
                    }
                    self.state = {**self.binding(), "phase": "owned", "receipt": receipt}
                    self.save_state()
                ...
                def cleanup(self):
                    ...
                    self.require(self.state is not None and self.state["receipt"] == self.expected)
                    ...
                    if receipt["common_path"] is not None:
                        common = self.track(self.open_path(receipt["common_path"]))
                        self.require(self.identity(os.fstat(common)) == receipt["common"])
                        self.not_ancestor(receipt["checkout"], receipt["common_path"])
                    if claimed is None:
                        if original is None or self.identity(original) != receipt["checkout"]:
                        ...
                        if moved is None or self.identity(moved) != receipt["checkout"]:
                            try:
                                self.rename(scope, self.claim_name, parent, self.name)
                            except OSError:
                                pass
                            raise ValueError("Source claim identity changed.")
    ...
    public static function script(): string
    {
        return self::definitions()."\n".<<<'PYTHON'
            try:
                with TransferSource(json.load(sys.stdin)) as source:
