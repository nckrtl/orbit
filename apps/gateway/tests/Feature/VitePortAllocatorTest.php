...
it('does not remove the destination service during cleanup when both nodes use the same checkout path', function (): void {
    ...
    $runtime = Mockery::mock(\App\Domain\Processes\ProcessRuntimeManager::class);
    $runtime->shouldNotReceive('remove');
    $schedules = Mockery::mock(\App\Domain\Schedules\ScheduleRuntimeManager::class);
    new \App\Infrastructure\AppInstances\NativeAppInstanceTransferRuntime($runtime, $schedules)->cleanupSourceArtifacts($instance, $source, '/apps/main');
});
