...
describe(RemoteToolCommandRunner::class, function (): void {
    it('uses the node wireguard address and fixed argv execution', function (): void {
        ...
        expect($result->exitCode)
            ...
            ->toBe('/tmp/orbit/id_ed25519')
            ->and($executor->connection?->knownHostsFile)
            ->toBe('/tmp/orbit/known_hosts')
            ->and($executor->command?->arguments)
            ->toBe(['sudo', 'apt-get', 'update'])
            ->and($executor->command?->input)
            ->toBeNull()
            ->and($executor->command?->protectedInput)
            ->toBeNull();
    });
        ...
        expect(fn () => $runner->execute(remote_tool_runner_node('10.8.0.8'), ['sudo', 'apt-cache', 'policy']))
            ->toThrow(function (ToolManagerException $exception) use ($stdoutSentinel, $stderrSentinel): void {
                expect($exception->step)
                    ...
                    ->not->toContain($stdoutSentinel, $stderrSentinel)->and(print_r($exception, return: true))
                    ->not->toContain($stdoutSentinel, $stderrSentinel);
            });
    });
});

function remote_tool_runner_node(?string $wireguardIp): Node
{
    return new Node([
        'name' => 'tool-node',
        'status' => 'active',
