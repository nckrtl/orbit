...
use App\Actions\AppInstances\Dependencies\UpdatePnpmDependenciesAction;
...
const PNPM_UPDATE_VP_PATH = '/home/orbit/.local/share/vite-plus/bin/vp';

function pnpm_update_instance(bool $production = false, string $path = '/home/orbit/project'): AppInstance
{
...
function pnpm_update_keys(): void
{
 ...
 */
function pnpm_update_ssh(array $calls): void
{
    pnpm_update_keys();
    $index = 0;
...
function pnpm_update_never_ssh(): void
{
    pnpm_update_keys();
    mock(SshExecutor::class)->shouldReceive('execute')->never();
}

function pnpm_update_probe_command(RemoteCommand $command, string $path = '/home/orbit/project'): bool
{
    expect($command->arguments)->toBe(['/usr/bin/python3', '-I', '-', $path]);
    expect($command->input)->toBe(PnpmDependencyUpdatePresenceProgram::render());
...
function pnpm_update_update_command(RemoteCommand $command, bool $expectCancelled = false, string $path = '/home/orbit/project', string $vpPath = PNPM_UPDATE_VP_PATH): bool
{
    ...
    expect($command->arguments[5])->toBe(NpmDependencyUpdateProgram::render());
...
function pnpm_update_probe_present(string $version = '0.3.0', string $vpPath = PNPM_UPDATE_VP_PATH): CommandResult
{
    $vp = $version === ''
        ? 'null'
...
describe('bounded Vite+ pnpm dependency updates', function (): void {
        ...
        pnpm_update_ssh([
            [
                function (SshConnection $connection, RemoteCommand $command): bool {
                    ...
                    return pnpm_update_probe_command($command);
                },
                pnpm_update_probe_present(),
            ],
            [
                fn (SshConnection $connection, RemoteCommand $command): bool => pnpm_update_update_command($command),
                new CommandResult(0, '', '', 12, false),
            ],
        ]);

        $result = app(UpdatePnpmDependenciesAction::class)->execute(pnpm_update_instance());
    ...
    it('accepts every Orbit-managed Vite+ installation layout', function (string $vpPath): void {
        pnpm_update_ssh([
            [
                fn (SshConnection $connection, RemoteCommand $command): bool => pnpm_update_probe_command($command),
                pnpm_update_probe_present(vpPath: $vpPath),
            ],
            [
                fn (SshConnection $connection, RemoteCommand $command): bool => pnpm_update_update_command($command, vpPath: $vpPath),
                new CommandResult(0, '', '', 12, false),
            ],
        ]);
    ...
    it('runs Vite+ in a recorded root that contains a space', function (): void {
        ...
        pnpm_update_ssh([
            [
                fn (SshConnection $connection, RemoteCommand $command): bool => pnpm_update_probe_command($command, $path),
                pnpm_update_probe_present(),
            ],
            [
                fn (SshConnection $connection, RemoteCommand $command): bool => pnpm_update_update_command($command, path: $path),
                new CommandResult(0, '', '', 12, false),
            ],
        ]);

        $result = app(UpdatePnpmDependenciesAction::class)->execute(pnpm_update_instance(path: $path));
    ...
    it('skips an absent pnpm ecosystem without running Vite+', function (): void {
        pnpm_update_ssh([
            [
                fn (SshConnection $connection, RemoteCommand $command): bool => pnpm_update_probe_command($command),
    ...
    it('refuses production before SSH', function (): void {
        pnpm_update_never_ssh();

        $result = app(UpdatePnpmDependenciesAction::class)->execute(pnpm_update_instance(production: true));
    ...
    it('refuses unsafe instance identity before SSH', function (): void {
        ...
        $instance = pnpm_update_instance(path: 'relative/root');
        $result = app(UpdatePnpmDependenciesAction::class)->execute($instance);

        expect($result->errorCode)->toBe('dependencies.unsafe_source');
        expect($result->mayHaveMutated)->toBeFalse();
    });

    it('fails an incomplete pnpm project without mutation', function (): void {
        pnpm_update_ssh([
            [
                fn (SshConnection $connection, RemoteCommand $command): bool => pnpm_update_probe_command($command),
    ...
    it('fails a missing Vite+ installation as unsupported delegation before mutation', function (): void {
        pnpm_update_ssh([
            [
                fn (SshConnection $connection, RemoteCommand $command): bool => pnpm_update_probe_command($command),
                pnpm_update_probe_present(version: ''),
            ],
        ]);
    ...
    it('fails an unverified Vite+ version as unsupported delegation before mutation', function (): void {
        pnpm_update_ssh([
            [
                fn (SshConnection $connection, RemoteCommand $command): bool => pnpm_update_probe_command($command),
                pnpm_update_probe_present(version: '0.2.6'),
            ],
        ]);
    ...
    it('rejects malformed probe receipts and Vite+ paths', function (string $stdout): void {
        ...
        'malformed version' => ['{"status":"present","vp":{"path":"'.PNPM_UPDATE_VP_PATH.'","version":"v0.3.0"}}'],
    ...
    it('rejects probe stderr and truncated output', function (): void {
        ...
        expect(app(UpdatePnpmDependenciesAction::class)->execute(pnpm_update_instance())->errorCode)
    ...
    it('passes the cancellation callback through both remote calls', function (): void {
        pnpm_update_ssh([
            ...
            [
                fn (SshConnection $connection, RemoteCommand $command): bool => pnpm_update_update_command($command, expectCancelled: true),
                new CommandResult(0, '', '', 12, false),
            ],
        ]);

        $cancelled = fn (): bool => false;
        $result = app(UpdatePnpmDependenciesAction::class)->execute(pnpm_update_instance(), $cancelled);
