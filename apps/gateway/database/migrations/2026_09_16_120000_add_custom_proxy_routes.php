...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::table('routes', static function (Blueprint $table): void {
            $table->string('kind', 32)->default('app');
        });
        ...
        Schema::create('route_custom_proxies', static function (Blueprint $table): void {
            $table->unsignedBigInteger('route_id')->primary();
            $table->foreignId('node_id')->constrained()->restrictOnDelete();
            $table->foreignId('process_id')->nullable()->constrained('processes')->restrictOnDelete();
            $table->string('upstream', 253);
            $table->timestamps();
        ...
        DB::statement(<<<'SQL'
            ...
            WHEN (
                NEW.kind NOT IN ('app', 'custom_proxy')
        ...
        DB::statement(<<<'SQL'
            ...
            WHEN (
                NEW.kind <> OLD.kind
                OR NEW.kind NOT IN ('app', 'custom_proxy')
                OR (NEW.kind = 'app' AND NEW.app_id IS NULL)
        ...
        DB::statement(<<<'SQL'
            CREATE TRIGGER route_targets_reject_custom_proxy
            BEFORE INSERT ON route_targets
            WHEN (SELECT kind FROM routes WHERE id = NEW.route_id) = 'custom_proxy'
            BEGIN
                SELECT RAISE(ABORT, 'A custom proxy Route cannot own App instance targets.');
            END
            SQL);
    ...
    {
        throw new RuntimeException('Custom proxy Route contracts cannot roll back.');
    }
};
