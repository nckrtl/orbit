...
describe("fallbackPollInterval", () => {
    it("reloads task queries every five minutes while realtime is live, as a safety net", () => {
        expect(fallbackPollInterval("live")).toBe(TASK_SAFETY_POLL_MS);
        expect(TASK_SAFETY_POLL_MS).toBe(300_000);
    });

    it("polls task queries every 30 seconds while realtime is down or not configured", () => {
