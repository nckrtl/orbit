#!/usr/bin/env bash
# Provision without settings stores SQL null; provision with settings round-trips raw overrides.
...
orbit node:provision app-dev --json >/dev/null
[[ "$(sql_node_settings app-dev)" == null ]] || fail "provision without settings stored overrides"

out=$(orbit node:provision app-dev \
