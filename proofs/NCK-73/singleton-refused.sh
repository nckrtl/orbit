...
echo "$gateway" | grep -q '"error"' || fail "Gateway accepted a second metrics claim: $gateway"
...
echo "singleton: second claim refused (cli=metrics.assignment_exists, gateway refused, app-prod roles=$roles)"
