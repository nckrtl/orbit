...
[[ "$action" == metrics-node-fails-closed && "$event" =~ ^(EXIT|INT|TERM)$ \
  && "$window" =~ ^(post-record|post-mutation)$ && "$deadline" =~ ^[1-9][0-9]*$ ]]
expected=130
...
env ORBIT_E2E_PROOF_ROOT="$proof_root" ORBIT_E2E_ORB7_MODE=signal \
  ...
  ORBIT_E2E_ORB7_CHECKPOINT="$checkpoint" python3 - "$proof_root/metrics-node-fails-closed.sh" <<'PY' &
import os
import signal
import sys
