...
The [Route reference](/reference/routes) defines initial private traffic projection, Node TLD reconciliation for generated private Routes, Cluster activation and deactivation reconciliation, and the refusal boundary for Route, Cluster TLD, membership, and access changes that still need coordinated runtime and Laravel URL reconciliation.
