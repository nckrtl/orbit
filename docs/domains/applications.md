...
Active means Orbit prepared the source, PHP runtime if needed, supported Laravel configuration, Route, and the Project's setup steps. An empty setup list counts as success. The application can still fail for a reason outside those steps.

A setup command that exits non-zero removes the new Instance after running the teardown list. Retrying creation for an active Instance with a recorded profile returns it unchanged and does not run setup again. `instance:setup` runs the current setup list against an Instance that already exists.
