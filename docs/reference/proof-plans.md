...
| `mutates` | boolean | The plan changes reusable node state, so `promote` refuses the proved topology and closeout refreshes the snapshot from the merge commit | `false` |
