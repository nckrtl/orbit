...
| `stale` | A runtime or proof-contract input changed | Preserve the old evidence and review record, release its proof lease, then `prove` |
| `indeterminate` | A classification, identity, current-main, manifest, plan, or completeness gate failed | Resolve the listed errors, preserve the old evidence and review record, release its proof lease, then `prove` |
