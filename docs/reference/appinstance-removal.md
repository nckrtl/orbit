...
Orbit accepts removal of one independent development checkout or one production AppInstance. The Gateway refuses a development checkout with linked worktrees before mutation, including when `--force` is present. It also refuses a development worktree AppInstance. An AppInstance that still needs manual source migration returns `instance.migration_required` before removal preflight can accept it.
...
Repeating the same removal request resumes the first unfinished step. The Gateway refuses a changed force value or a request owned by another operation. Before further development source deletion, it revalidates the recorded checkout under the same Node lock. A retry after shared target removal preserves the survivor set without restoring the departing target. A retry after Route deletion continues without recreating the Route or reclaiming its hostname. If final completion cannot commit, the same transaction restores the final member checkpoint and AppInstance row, so the identical public request remains model-bindable.
...
AppInstance removal does not activate development worktree or linked-worktree cascade removal, change App source settings, migrate a default source, adopt local source, delete branches, create production target pools, select a balancing policy, or reconcile unrelated Route changes. It retains production application content and the dedicated production user.
