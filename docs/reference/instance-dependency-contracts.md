...
npm requires a root dependency and a root `overrides` entry for the same name to share one spec. When that shared spec is an `npm:` alias, npm can lock and install the unaliased package under the declared name, and `npm ci` accepts the lock. The reader accepts that alias target only when the root `overrides` entry for the name is the same spec or `$name`, including an object override's `"."` value, and the locked identity is the declared name. The resolution then records the package npm installed, for example `vite@8.0.2`, while the requirement keeps the alias constraint. Any other alias identity mismatch fails.
