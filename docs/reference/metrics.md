...
A relocate that stops in a Metrics step reports the same code as `error.code`, with `details.step` set to `converge:metrics-<step>`. Adding or relocating the `gateway` role grants that Gateway access to the `vpn` and `metrics` nodes so this path can run. [ADR 0092](/decisions/0092-publish-private-dns-on-the-vpn-node-after-gateway-relocate) owns those rules.
