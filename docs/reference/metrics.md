...
A failure in another step records that step and the Metrics error code: `failed_step` is `converge:metrics-exporters`, `converge:metrics-cadvisor`, `converge:metrics-runtime`, or `converge:metrics-publication`, and `error_code` is the `metrics.*` code of the failure, such as `metrics.container_inspection_failed`. A step that already names itself, such as `converge:private-dns`, keeps its own name. Adding or relocating the `gateway` role grants that Gateway access to the `vpn` and `metrics` nodes so this path can run. [ADR 0092](/decisions/0092-publish-private-dns-on-the-vpn-node-after-gateway-relocate) owns those rules.
