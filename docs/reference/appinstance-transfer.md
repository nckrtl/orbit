...
The Git snapshot preserves local refs, symbolic ref targets and annotated tags, origin and effective repository configuration, and the exact index, including split-index data. The object pack contains all required objects, including blobs needed only by the index and unpublished history outside the current branch. Reconstruction does not check out files over working bytes. It removes source-specific repository paths and external object dependencies. If refs, HEAD, configuration, or index change during capture, the attempt fails instead of publishing mixed Git state.

Transfer preserves shallow-history boundaries and copies locally available alternate objects into the destination pack. It does not fetch missing objects. Repository-local ignore and attribute files remain effective without source paths; source hooks and filesystem-monitor settings are not copied. Submodule gitlinks are not supported. Capture refuses them before destination creation rather than copying source-dependent submodule administration. Each captured index, configuration scope, or portable Git file is limited to 16 MiB, and the snapshot manifest is limited to 96 MiB. Unsafe, unavailable, or oversized metadata stops transfer before cutover.
