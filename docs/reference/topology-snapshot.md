...
The rendered pools, Caddy sites, firewall rules, and DNS records then match the checkout. When `create-resources` returns no typed checkout path, `converge-app-prod-internal-tls.sh` writes `/etc/caddy/orbit-e2e-global.caddy` with a `local_certs` block outside the managed version; no build imports it. `internal-tls` on `app-prod` then runs before reproject. It removes a `fragments/00-orbit-e2e-global.caddy` that an older snapshot left in the managed version. On a Node whose Caddyfile a [Node Caddy build](/reference/caddy-configuration#node-caddy-build) wrote, it finds no fragment and changes nothing. Reproject builds each Node, and the first build backs up and replaces an older snapshot's fragment layout. Doctor then reports no Caddy drift.
