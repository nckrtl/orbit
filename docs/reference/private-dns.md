...
The Gateway installs the listener itself. The listener is a small release: `serve.php` and the Gateway classes it uses, with no Composer dependencies. Every publication that activates the listener sends the release from the Gateway's own code to the Node that holds `vpn`, over SSH when the roles split. The release lands in `/var/lib/orbit/private-dns/releases/<id>/`, where `<id>` is a digest of its files. A Gateway deploy that changes the listener code produces a new id. The next publication installs it with a `.manifest` of file digests, checks it with `serve.php --self-test`, and points the unit at it. `--self-test` checks `ext-sockets` and `ext-pcntl` and loads every class. A release whose files no longer match its manifest is reinstalled, and the listener restarts on it. The publication keeps the previous release for rollback and removes older ones. [ADR 0149](/decisions/0149-install-the-private-dns-listener-from-the-gateway) records this decision.
