...
`gateway.orbit`, `metrics.orbit`, and `reverb.orbit` are reserved platform names. A Route cannot own them. `gateway.orbit` answers with the WireGuard address of the Node that currently holds the active `gateway` role. Relocating that role republishes the record; see [Relocate the gateway role](/solutions/relocate-gateway-role).
