...
| Destroy | Clear the site publication, publish DNS and Caddy without the site, then remove the Orbit CA leaf. A failed step keeps the leaf and leaves the Route `failed` with `failed_step`; destroy it again to retry. Instance Routes stay unchanged. |
