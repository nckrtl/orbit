...
AppInstance removal is the coordinated target-clear exception. After complete source preflight, the Gateway marks the fixed removal set `removing` and clears each target. A shared production Route converges with its remaining targets. The Gateway deletes every final-target Route and releases its hostname before deleting source. During coordinated normal or forced development removal only, the short interval before that deletion propagates serves the exact transient unavailable response. The [AppInstance removal reference](appinstance-removal.md) owns that response, the removal order, and retry behavior.
