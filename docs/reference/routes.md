...
When the target AppInstance has no recorded source profile, the Gateway refuses the change with `instance.source_profile_missing` and HTTP 409 before any Route or projection change. [Applications](../domains/applications.md#recover-a-missing-source-profile) describes the creation retry that recovers the profile.
...
For a production AppInstance, the Gateway preflights the recorded production environment location and renders the complete stored configuration against the candidate hostname. It resolves `{{app_instance.hostname}}` to that hostname, preserves unrelated stored values and literal application keys, and replaces only the production-home `.env`. The transition does not run Composer, Artisan, framework bootstrap, a cache command, a deployment, or a worker or service restart. The operating agent adds any required application cache or process command to a separate deployment step. A stale cached application URL or an HTTP error does not block an infrastructure-valid hostname transition. [AppInstance environment variables](environment-variables.md#synchronize-during-a-hostname-change) owns the file boundary.
