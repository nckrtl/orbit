...
Production sample hydration and HTTPS verification use the recipe's routing placement. The shared-cluster recipe probes through the Gateway Router at `10.44.0.1`. The cold recipe has no Gateway Router, so its probes connect to the production Node's loopback address. Both keep the recorded Route domain and trusted CA verification. A failed request never switches to another destination.
