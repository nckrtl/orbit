...
An App is Orbit's stable record for one application. It owns the repository
URL, main branch, and relative web root that later AppInstances inherit as
source defaults.
...
`app:new` is an idempotent creation command. Repeating it with the same name,
slug, repository, main branch, root, and defaults returns the existing App. An
omitted branch is not resolved again during that retry.
