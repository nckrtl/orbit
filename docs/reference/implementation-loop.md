...
Each project job checks out the triggering branch by name with full history so Pest can write its TIA graph. Detached HEAD and shallow clones skip that write. After Composer install, the job restores that project's portable PHPStan result cache at `vendor/phpstan/cache/resultCache.php` from GitHub Actions cache, then saves it only after `composer check` succeeds. It does not cache PHPStan's compiled container or Larastan's migration cache. The job restores that project's `.orbit-tia` directory from GitHub Actions cache, runs impacted tests with `ORBIT_TIA_DIRECTORY=.orbit-tia`, and saves the graph only after Pest succeeds. Guidance checks still record into `vendor/.orbit-guidance-tia` and are not cached. Hosted CI does not call `bin/tia-cache`. Incus acceptance remains the independent review proof.
