...
Automatic task workspace provisioning creates independent clones, which do not share this Git store. Before an agent starts, task preparation transfers the main publications to the clone and runs the Project setup steps, normally `bin/bootstrap`, with `ORBIT_MAIN_CACHE_STORE`. The clone seeds from those publications and never publishes back.
