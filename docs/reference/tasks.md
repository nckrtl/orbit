...
When an implementer is idle, done, or asking for input, the Gateway reads `composer check` from tool activity: the command ran, the exit code is 0, and no edit, write, or patch follows it. It also reads `composer.json` at the workspace root, which must define a `check` script, and the implementer's run receipt. A pending input fails on its own. Jev is not asked about the implementer.

Before each implementer turn, the Gateway installs the run script at `.git/orbit/run`, writes `.git/orbit/turn.json` with the role of the turn, and removes any earlier receipt. Git never tracks `.git/orbit/`. The implementer ends its turn with `.git/orbit/run --outcome=ready_for_review --summary="…"`, or with `--outcome=blocked` when something outside the brief stops it. The script refuses other outcomes, an empty summary, a repeated flag, and unknown arguments. It writes `.git/orbit/run.json` atomically. [ADR 0121](/decisions/0121-end-agent-turns-with-a-run-receipt) records the decision.

When the implementer stops, the tick reads the receipt over SSH, stores it as a task comment with its content hash, and removes it. A receipt read again after a crash has the same hash and is stored once. `blocked` asks for assistance with the agent's summary. A missing receipt, or one whose outcome does not fit the turn, fails the `run_receipt` item. `ready_for_review` passes that item, and the code checks decide. An unreachable workspace counts as a communication failure, not a missing receipt.
...
The run script instructions name the `ready_for_review` and `blocked` commands. The reminder does not say that the thread is blocked and does not ask for an `assistance_requested` comment. An implementer reports a blocker with a `blocked` receipt. A blocker the reviewer names in its reply fails the next evaluation.
