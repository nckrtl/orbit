...
Before the first implementer of a group starts, Orbit runs the Project's configured baseline check on the fresh workspace. Set `task_baseline_check` with `PATCH /api/v1/projects/{project}` or `orbit project:update <project> --baseline-check=COMMAND`. Clear it with `task_baseline_check: null` in the API or `--clear-baseline-check` in the CLI. When it is unset, Orbit runs setup steps and then passes the baseline without invoking `composer check`.

The upgrade migration sets existing Projects to `composer check`, preserving their current baseline gate. Projects created after the upgrade have no configured check unless an operator sets one.

Project setup runs before dependency preparation so it can configure credentials or install dependencies itself. Orbit prepares Composer dependencies when the configured command references Composer or `vendor/`: it walks tracked `composer.json` files and runs `composer install --no-interaction --prefer-dist` where a sibling lockfile exists and `vendor/autoload.php` is missing. It prepares JavaScript dependencies when the command references Bun, npm, pnpm, Yarn, Node, Vite+, or `node_modules`: it walks tracked `package.json` files and runs `vp install --frozen-lockfile` where a supported lockfile exists and `node_modules` is missing. Both guards skip projects whose dependencies are already installed. Commands that do not indicate either dependency family receive no automatic install prep.

A failed setup or dependency install asks for assistance and names that step. A failed baseline command reports `The Project baseline check failed` with its exit code and output. If command output indicates missing `vendor/` or `node_modules` dependencies, assistance reports `Project dependencies appear to be missing` rather than calling the branch broken. Fix the cause, then cancel and create the group again. Handoff checks still run `composer check`. A task's `check` shows the latest run, with `kind` `baseline` or `handoff` and the `failed_step`.
