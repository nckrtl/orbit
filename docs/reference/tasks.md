...
Before the first implementer of a group starts, Orbit runs a baseline check on the fresh workspace: the Project's setup steps, a Composer dependency install, then `composer check`. The first implementer starts only when it passes.

The automatic install walks tracked `composer.json` files and runs `composer install --no-interaction --prefer-dist` in each project that has a sibling `composer.lock` but no `vendor/autoload.php`. It skips projects whose dependencies are already installed. Project setup runs first, so it can configure Composer authentication or other settings needed by the install. A setup step can also install dependencies itself, and the automatic step skips them when it sees `vendor/autoload.php`.

A failed setup or dependency install asks for assistance and names that step; a failed automatic install reports `Composer dependency installation failed`. If `composer check` output indicates a missing `vendor/bin/...` executable or a failed load of `vendor/autoload.php`, assistance names missing Composer dependencies rather than calling the branch broken. Other check failures report the real composer check failure. Fix the cause, then cancel and create the group again. A task's `check` shows the latest run, with `kind` `baseline` or `handoff` and the `failed_step`.
