...
A claim can stop after it provisions the `task-{group id}` workspace and before it attaches it. That workspace keeps its name and its `task-{group id}` branch, so the next claim resumes it, and cancellation and completion find it by that name and branch when the group holds no Instance. An Instance that only shares the name, with another branch, is never removed.

When a group is cancelled while its claim runs, the claim removes the workspace it provisioned, or the part of it that exists when provisioning fails, and the group stays `cancelled`. When that claim stops before it can, each tick removes the `task-{group id}` workspace of a `cancelled` or `completed` group that holds no Instance and was reserved longer ago than `ORBIT_TASKS_RESERVED_TIMEOUT_SECONDS`, or never. A tick attempts at most five such removals. A failed removal goes to the application log and is retried on a later tick.
