...
A development AppInstance owns one Git source with layout `checkout` or `worktree`. The reserved `default` identity uses the App `default_branch` unless creation selects another branch explicitly; every other identity uses its matching remote branch or falls back to `default_branch`. The Gateway then selects any required PHP runtime from the source, configures a detected Laravel application, and provisions the AppInstance's sole Route. [Applications](domains/applications.md) owns stable source identity, explicit branch selection, manual migration, and application configuration, [PHP runtime](reference/php-runtime.md) owns runtime selection and settings, and [Routes](reference/routes.md) owns the private traffic path and mutation guards.
