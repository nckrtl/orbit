...
A development AppInstance owns one Git source with layout `checkout` or `worktree`. A standalone production AppInstance owns a recorded user and home, starts from one initial repository checkout, and leaves later deployments to the operator. The Gateway selects any required PHP runtime from the source and provisions the AppInstance's sole Route. [Applications](domains/applications.md) owns source identity, branch selection, production placement, migration boundaries, and application configuration. [AppInstance removal](reference/appinstance-removal.md) owns development source deletion, fixed-set cascades, retained production content, Route cleanup, and retry. [PHP runtime](reference/php-runtime.md) owns runtime selection and settings, and [Routes](reference/routes.md) owns the private traffic path and mutation guards.
