...
A development AppInstance owns one Git source with layout `checkout` or `worktree`. The Gateway then selects any required PHP runtime from the source, configures a detected Laravel application, and provisions the AppInstance's sole Route. [Applications](domains/applications.md) owns source identity, branch selection, migration boundaries, and application configuration. [AppInstance removal](reference/appinstance-removal.md) owns development source deletion, retained production content, Route cleanup, and retry. [PHP runtime](reference/php-runtime.md) owns runtime selection and settings, and [Routes](reference/routes.md) owns the private traffic path and mutation guards.
