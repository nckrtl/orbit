...
When a development AppInstance sits on a Node with the active `app-dev` role, idle HTTP silence stops its desired-running Processes, and the next request starts them again. The [hibernation page](reference/app-dev-runtime-hibernation.md) states that contract.
