...
When a development AppInstance sits on a Node with the active `app-dev` role, idle HTTP silence stops its desired-running Processes that are not keep-alive, and the next request starts the desired-running group. After a longer idle window the Gateway deletes reconstructable checkout dependencies and restores them before that start. The [hibernation page](reference/app-dev-runtime-hibernation.md) states that contract.
