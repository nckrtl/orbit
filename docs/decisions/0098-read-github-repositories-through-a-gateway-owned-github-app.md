...
- The manifest requests `Contents: read` and `Metadata: read`, turns webhooks off, and marks the App public so that accounts other than its owner can install it. The operator chooses the App name, because GitHub App names are unique across GitHub.
