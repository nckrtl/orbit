...
- The manifest requests `Contents: read` and `Metadata: read`, turns webhooks off, and marks the App public so that accounts other than its owner can install it. The convert exchange sends an empty JSON body, because Laravel's empty `post()` encodes as `[]` and GitHub's convert schema rejects an array. The posted manifest also omits `default_events` rather than sending an empty list. The operator chooses the App name, because GitHub App names are unique across GitHub.
