---
title: "ADR 0149: Install the private DNS listener from the Gateway"
sidebarTitle: "0149 Install the private DNS listener from the Gateway"
...
# ADR 0149: Install the private DNS listener from the Gateway
...
- Every publication that activates the listener sends the release, the service unit, and a socket unit to the Node that holds `vpn`. It uses the same script locally or over SSH. It writes a `sha256sum` manifest with each release in `/var/lib/orbit/private-dns/releases/<id>/`. It reinstalls a release whose files do not match that manifest, and it installs only a release that passes `serve.php --self-test`, which checks `ext-sockets` and `ext-pcntl` and loads every class. It keeps the previous release and removes older ones.
- `orbit-private-dns.socket` binds UDP and TCP port 53 on the VPN DNS address with `FreeBind=yes` and passes both sockets to `orbit-private-dns.service`. It does not wait for the WireGuard tunnel: `FreeBind=yes` binds the address before it exists, and ordering the socket after `wg-quick@orbit.service` forms a boot cycle through `sockets.target` and `basic.target`. The service orders itself after the tunnel but does not require or want it, so a query that activates the listener never starts a tunnel that an operator stopped. The service runs `serve.php` from the current release. A restart keeps the sockets open, so queries that arrive during it wait instead of failing. The listener finishes the query in hand when it gets `SIGTERM`.
...
- The listener classes must stay free of framework code. Every `App\` class that a listener class names joins the release, so the listener resolves requesters only from the published catalog. The database lookup that only tests used moved into test support. The release test runs the release without the Composer autoloader.
