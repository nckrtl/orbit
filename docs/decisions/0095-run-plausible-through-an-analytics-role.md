...
- The one generated secret is Plausible's `SECRET_KEY_BASE`. It is stored as a protected setting, reused each time the role converges, never logged, and never returned over the API. The two connection URLs are derived at converge time and reach the container only through its environment.
