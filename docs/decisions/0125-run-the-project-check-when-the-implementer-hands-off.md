...
Before the first implementer of a group starts, Orbit runs a baseline check on the fresh workspace. The check script first runs the Project's setup steps in order, then installs Composer dependencies, then runs `composer check`, with the same detached process and states. The dependency step walks tracked `composer.json` files and runs `composer install --no-interaction --prefer-dist` in each project that has a sibling `composer.lock` but no `vendor/autoload.php`. This guard skips projects whose dependencies are already installed. Project setup runs first so it can prepare Composer credentials or configuration; a setup step may also install dependencies, in which case the guard skips that project. An agent never starts on a broken checkout. When setup or dependency installation fails, the group asks for assistance and names the failed step; the dependency failure reason says that Composer dependency installation failed. When `composer check` output shows a missing `vendor/bin/...` executable or a failed load of `vendor/autoload.php`, assistance says that Composer dependencies appear to be missing instead of calling the branch broken. Other check failures retain the check failure reason. The operator fixes the setup, dependencies, or branch, then cancels and creates the group again. A group whose implementer already started skips the baseline.
