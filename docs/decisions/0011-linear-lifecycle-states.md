...
Orbit needs to accept rough issues without allowing the delivery coordinator to start them before
...
- `In Progress`: the delivery coordinator has selected the issue and started or resumed its Herdr
  lifecycle. The delivery coordinator moves `Todo` to `In Progress` and reads it back before creating
...
When capacity is below three, the delivery coordinator scans `Todo` in the established queue order
...
The delivery coordinator continues to the next eligible issue.
...
- `FIX` starts fresh correction-planner and reviewer rounds while the issue
  remains `In Progress`.
- `BLOCK` moves the issue to `Blocked`. The delivery coordinator adds a Linear comment naming the
...
the execution queue without deleting its retained assets. On re-selection the delivery coordinator:
...
The delivery coordinator moves an issue to `In Review` before dispatching independent PR review.
...
- Raw issue capture is safe because the delivery coordinator claims only `Todo` issues.
