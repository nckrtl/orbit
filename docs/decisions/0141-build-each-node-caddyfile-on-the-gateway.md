...
- A role's row serves before its certificate step runs, during a first convergence or a relocation. The Gateway therefore records, per Node, when the `websocket`, `analytics`, ProxyCli, or Metrics certificate step has placed its certificate, and forgets it at the removal that withdraws the site. Those sites render only while the record exists, so a concurrent build leaves them out instead of failing validation for every site on the Node. A `websocket` move keeps the site on the old Node until the new Node serves it and private DNS answers have moved, with the 31-second grace of a Route move. Private DNS names the new Node for `reverb.orbit` only after the new Node's build is live; until then it answers with the Node that still holds the site's certificate record, and the Gateway's own realtime client connects to that same Node. From then until the old Node's site is withdrawn and its Reverb has stopped, the Gateway broadcasts to both Reverb servers and its agent view subscriber listens on both, because clients that connected before DNS moved stay on the old server until the withdrawal build's Caddy reload closes their connections. A failed withdrawal leaves the move incomplete and both servers served until a retry finishes it. A migration records the certificate of every such site that renders at the cutover.
...
- Private DNS answers carry a 30-second TTL. A transition waits a grace period of 31 seconds after it moves an answer before it withdraws the old target. One operation waits once for all its Routes and does not hold the development projection owner while it waits; it reads each Route again before the withdrawal. The Route stores when DNS last moved, and the grace period counts from that move, so another change that moves the same Route's DNS during the wait restarts it rather than shortening it. The grace period covers resolvers that honor the TTL. Caddy finishes in-flight requests on reload and closes idle keep-alive connections, but a client with a longer DNS cache of its own can reach the withdrawn target until that cache expires.
...
- The Incus harness writes no Caddy files. It has no `internal-tls` step and no `local_certs` global options; its `converge-app-prod.sh` step only provisions `app-prod`. The first build on a snapshot Node backs up any fragment an earlier harness left behind.
- Doctor reads sites only from the single live Caddyfile. It reports `role.caddy_build_drift` when the live Caddyfile differs from a fresh render and lists the site sources on that Node. A Node whose live Caddyfile no build wrote, such as the fragment layout of an earlier release, reports that drift too; Doctor does not read fragments. Repeating any command that publishes one of those sites builds the Node again.
...
- A Node that holds `ingress` and a WireGuard-only site, such as `gateway` with `ingress` and `router`, fails the build when its first-row sites share the port. `gateway` with `router` and no `ingress` builds: its Router sites bind the WireGuard and LAN addresses.
