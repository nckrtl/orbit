...
- A role's row serves before its certificate step runs, during a first convergence or a relocation. The Gateway therefore records, per Node, when the `websocket`, `analytics`, ProxyCli, or Metrics certificate step has placed its certificate, and forgets it when removal takes the certificate away. Those sites render only while the record exists, so a concurrent build leaves them out instead of failing validation for every site on the Node. A migration records the certificate of every such site that renders at the cutover.
