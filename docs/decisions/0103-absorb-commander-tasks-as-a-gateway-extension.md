...
- A claimed group leaves `reserved` when the provisioner assigns an instance on a Node that still has capacity and T3 accepts the reviewer and first implementer spawn. A missing remote task branch does not block that assignment: the workspace is created from `default_branch`. Missing Nodes, incomplete App source defaults, or a refused `project.create` or `thread.create` leave the group `reserved` or `running` without thread ids so create does not fail closed on a down T3 server. A refused `thread.turn.start` after a successful `thread.create` still stores the thread id. A workspace-root collision adopts the existing T3 project instead of leaving the group without threads.
