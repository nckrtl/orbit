...
Operators then need those visits on the App instance page, next to the queue and the application log. The browser must not hold a Plausible management secret, and it must not be able to ask for another instance's site. The Gateway already reaches fleet services over WireGuard, as it does when it reads Prometheus through Grafana ([ADR 0088](/decisions/0088-cli-reads-display-metrics-from-grafana)). A later analytics product should not invent a second Plausible or a second event store.

## Decision

- The Gateway owns a fleet analytics driver interface. The first implementation talks only to the fleet's Plausible Community Edition Stats API. A later product can bind another implementation of the same interface; this decision does not add Plausible Cloud or a second Community Edition.
...
- Let the browser call Plausible or receive the Stats API key: rejected because a management secret would leave the Gateway, and a client could query any site the key can see.
...
- A later analytics product implements the same driver interface; it does not add a second Plausible.
