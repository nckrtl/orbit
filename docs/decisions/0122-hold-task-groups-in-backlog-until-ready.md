...
- `tasks:subtask:update` and `tasks:subtask:destroy` work only while the group is in `backlog`. Otherwise they fail with `tasks.not_in_backlog`. `tasks:subtask:create` keeps today's rule and appends to a group in any active status.
