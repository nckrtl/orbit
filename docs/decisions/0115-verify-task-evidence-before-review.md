...
The interim slice prepares Orbit task groups before agent start while the general project/instance setup feature remains separate. After source resolution, the task provisioner runs the repository's fixed `bin/bootstrap` command through pinned SSH. The scheduler consumes its exit status. Failure leaves the group queued and the instance available for retry; cancellation must not become a running group when preparation returns. No new readiness receipt is required.
...
Before this interim hook, the automatic `TaskWorkspaceProvisioner` path for `app.slug = orbit` stopped at source resolution and started the agent without bootstrap. The manual `bin/worktree-create` path already installed dependencies and seeded compatible caches. The verifier requires installed dependencies and reuses available history; it does not install them. The verification measurements above used an explicitly prepared checkout and do not prove automatic task-instance readiness.

Read-only inspection found Orbit registered as monorepo project 46 on the live Gateway. The inspected deployment, `12715ff83047c0e51f4b86618779b637b040471f`, had the task preparation gap. The current `CreateAppInstanceAction`, used by `instance:create`, still has no setup-step runner. [ADR 0030](/decisions/0030-complete-appinstance-provisioning-without-application-health-gates) already requires explicitly configured setup after provisioning, but leaves its implementation contract separate. The general feature requires code as well as project configuration.
