...
The runner keeps a private TIA directory per checkout and project outside the disposable source. It gives the snapshot a local branch because the installed Pest version does not save its dependency graph on detached HEAD. A nonblocking checkout lock prevents concurrent runners from updating the same cache. Pest owns dependency selection and invalidation; a missing or incompatible baseline can still require the full suite. Named evidence tests always execute with `--no-tia` so a cache hit cannot substitute for an observed result. Cold and warm runs must be measured separately, including a source edit that selects affected tests.
