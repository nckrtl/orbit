...
- Removing the `gateway` role removes the drop-in and reverts the `orbit` link it configured. Relocating it adds the route on the target and removes it from the source in the same way. A failure there fails the removal, as other removal steps do. After a failed removal, the role stays `failed` until the removal runs again and succeeds. After a failed source cleanup in a relocation, the role stays `active` on the target, and the error names the `--from` command that finishes the move.
