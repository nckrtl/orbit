...
- A Gateway that does not yet expose deployment history, database connection users, or Node metrics must render those panes as "Not available on this Gateway yet." rather than fail the screen. `App\Support\Tui\Sources\DeploymentsSource`, `DatabaseUsersSource`, and `NodeMetricsSource` are the seams that carry those requests once wired; `Null*Source` implementations stand in until a Gateway consistently exposes them from this screen.
...
- Deployment history, database connection users, and Node metrics panes silently degrade to "Not available on this Gateway yet." on a Gateway that has not yet wired `DeploymentsSource`, `DatabaseUsersSource`, or `NodeMetricsSource` to real requests, even after the underlying Gateway endpoints exist; closing that gap is a follow-up to this decision, not part of it.
