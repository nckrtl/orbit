...
- Caddy's certificate cache is shared across listeners. A client that asks the public listener for a private hostname in SNI completes the TLS handshake with that private site's certificate. Its HTTP request then matches no site on that listener, so Caddy answers with an empty `200` response. The client learns the private hostname from the certificate and gets no content.
- A private site on an Ingress Node refuses a LAN client whose address is outside private and shared address space.
- A container on a Node can no longer reach that Node's WireGuard-only sites, because its Docker bridge source address is outside the VPN subnet. That covers `gateway.orbit` and `metrics.orbit` on the Gateway Node, `reverb.orbit`, `analytics.orbit`, the ProxyCli collector `collector.cli-proxy-api.orbit`, and the service metrics scrape site. Containers on other Nodes still reach them, because their traffic leaves with the Node's WireGuard address. No Orbit flow depends on the same-Node case.
