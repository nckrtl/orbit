...
- The Gateway treats a public edge as live when `publication` is `public`, the Route is `active` or `activating`, the Cluster is eligible (active Cluster, one active Ingress, one active Router, Cluster scope), and activation has reached the public handler or has finished (`replacement_step` is null or ranks at `public-activated` or after). Live Caddy and Ingress firewall follow that derived state.
