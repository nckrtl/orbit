...
- Add `RoleName::WebSocket` (`websocket`) as a singleton, mutable role, assignable during provisioning, with no role conflicts, so it combines with `gateway` and `vpn` today. [ADR 0095](/decisions/0095-relocate-relocatable-singleton-roles) moves the assignment with `orbit node:role:relocate`; add-then-remove is not the supported path.
...
- Moving the websocket role to a dedicated Node uses `orbit node:role:relocate <node> websocket --force`. Relocate keeps the generated Reverb identity and repoints the hostname, certificate, DNS record, and Caddy site. Role prerequisites on that Node include `php-curl` and `php-xml`, so `composer install` does not depend on the Gateway PHP stack already being present.
...
- ADRs: amends [ADR 0084](/decisions/0084-broadcast-record-changes-through-reverb)'s deployment decision; the [ADR 0080](/decisions/0080-add-node-owned-custom-proxy-routes) custom proxy Route kind is unaffected and remains available for other Node-local services; [ADR 0095](/decisions/0095-relocate-relocatable-singleton-roles) owns the relocate path
- Detail: [Realtime events](/reference/events), [Realtime events with Reverb](/solutions/realtime-reverb), [`node`](/cli/node)
- Verify: Gateway role registry, baseline converge/remove/removeUnreachable, publication, and credential tests; `NodeBootstrapPackageCatalog` and role-prerequisite package lists for `websocket`, `app-dev`, and `app-prod`; `RealtimeConnection`, `RecordEventBroadcaster`, `RealtimeConfigController`, and channel-auth tests; CLI `node:role:add`/`node:role:relocate`/`node:role:remove` and `realtime:show` contract tests
