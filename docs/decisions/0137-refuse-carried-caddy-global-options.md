...
description: "Proposed. Superseded by ADR 0140. Orbit writes the only Caddy global options block on a Node. A publisher refuses a candidate when a fragment it carries forward opens its own global block, and names the fragment, the options, and the file to edit."
---
...
Proposed. Superseded by [ADR 0140](/decisions/0140-build-each-node-caddyfile-on-the-gateway), which renders each Node's whole Caddyfile on the Gateway and carries no fragment forward.
