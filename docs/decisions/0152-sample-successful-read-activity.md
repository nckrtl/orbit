...
description: "Proposed. The Gateway records every mutating request and every failed request as before, but keeps a successful read only once per command, caller, target, and minute. Schedule operations and credential reads are always kept."
---

# ADR 0152: Sample successful read activity

The Gateway keeps an Activity for every request that can change something and for every request that fails. A successful read (`GET` or `HEAD`) is kept only when it is the first read of that command, from that caller, of that target in the last 60 seconds. Schedule operations and credential reads are always kept.
...
- A successful `GET` or `HEAD` request is recorded when it is the first successful read of the same command, from the same caller address, of the same request path in the last 60 seconds. The path names the target, such as the Process whose logs were read or the database connection whose schema was read. The Gateway keeps this window in its cache with an atomic add. Other successful reads in the window write nothing.
- Every `schedule:*` operation is always recorded, which keeps the promise of ADR 0013. Doctor sends `POST`, so each run is still recorded, which keeps the promise of ADR 0004.
- A successful read of a route whose name ends in `:credentials` is always recorded.
- When the sampling cache fails, the Gateway records the read and logs a warning. Sampling never fails a read.
...
- A polling client writes about one row per command and target per minute instead of one per poll. A Schedule list still writes a row per poll. The web app's polls and agents that list records now cost almost no SQLite writes.
...
- `activity:list`, `activity:show`, and the MCP activity tools keep working, because the table and its fields do not change. Doctor does not read the log.
