...
- The staged `--no-dev` install must include every package a packed command needs at runtime. Laravel Zero lists `illuminate/http` as require-dev only, so the CLI must require it itself and bind `Illuminate\Http\Client\Factory`. Packed `orbit top` and `realtime:tail` authorize the Reverb channel through Laravel's HTTP client after the WebSocket connects.
...
- Rewriting channel authorization onto Guzzle or the PHP SDK to avoid `illuminate/http`: rejected because Laravel Zero's `http` component is the documented way to ship Laravel's HTTP client in a PHAR, and the CLI already uses `Http` for this call.
...
- A packed binary that omits `illuminate/http` fails after the fleet Dashboard loads: `Target class [Illuminate\Http\Client\Factory] does not exist` when it authorizes the realtime channel. The PHP source checkout still works because require-dev pulls the package in.
...
- Verify: `apps/cli` contract tests for the builder usage, artifact names, host assignment, and production `illuminate/http` binding; `bin/orbit-build-cli-binary linux x64` on a machine with PHP 8.5, Composer, rsync, and zlib; `bin/orbit-build-cli-binary mac arm` on mini; packed `orbit top` reaches **live** after realtime auth
