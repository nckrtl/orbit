...
`profile` sends one GET from this machine and reports timings. Name an absolute HTTP or HTTPS URL, or pass `--instance` to profile an Instance at the URL the Gateway records for it. [Gateway profiles](/cli/gateway) remain a separate local configuration family.

The request always leaves this machine. The command never opens an SSH session and never asks the Gateway to issue it; with `--instance` it asks the Gateway only where to send it, and verifies that host against the active profile's Orbit root certificate, because an Instance domain presents an Orbit CA leaf.
...
| --- | --- | --- |
| `--instance=ID` | none | Profile the Instance with this numeric ID, at the URL the Gateway records for it. Refuses a `url` argument at the same time. The request carries `X-Orbit-Probe: 1`, which a development Instance's site answers without waking the Processes hibernation halted and without counting as the activity that keeps them running, so profiling a sleeping instance leaves it asleep. |
