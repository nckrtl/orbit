...
Every other command sends its request to the active Gateway. The `gateway` family manages the local profiles that name a Gateway, pins its root certificate authority (CA), installs that certificate in the operating-system trust store, and shows whether the Gateway answers.
...
Every command accepts `--json`. `gateway:add`, `gateway:use`, and `gateway:remove` change only local configuration; `gateway:status` and `gateway:trust` contact the Gateway named by the active profile.
...
An unknown name returns `gateway.profile_not_found`.
...
```bash
orbit gateway:remove <name> [--force]
```
...
orbit gateway:remove default --force
```
