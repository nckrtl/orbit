...
An interactive call shows a preview and asks for confirmation, defaulting to No. Interactive decline, Ctrl-C, or EOF exits with `input.cancelled` and makes no changes. A non-interactive or `--json` call without `--force` fails with `metrics.confirmation_required`. `--purge-data` without `--force` fails with `metrics.force_required`, in every mode.
