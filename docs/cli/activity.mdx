...
- A text or structured field is stored as `[REDACTED]` when any word of its name is `key`, `token`, `secret`, `password`, `passwd`, `passphrase`, `credential`, `bearer`, or `pem`, singular or plural, in any case or separator style. The joined forms `apikey`, `appkey`, `authtoken`, `accesstoken`, and `privatekey` count too. For example, `cliproxy_management_key`, `api_key`, and `webhookSecret` are redacted, and `valkey_connection` is not. A new field that matches this rule is redacted without a code change.
- Only audited public fields are exempt: `public_key`, `wireguard_public_key`, and `host_key_fingerprint`. Numbers, booleans, and null stay, because they hold counts and flags such as token usage.
