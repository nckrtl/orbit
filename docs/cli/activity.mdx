...
- A text or structured field is stored as `[REDACTED]` when any segment of its name ends with `key`, `token`, `secret`, `password`, `passwd`, `passphrase`, `credential`, `bearer`, or `pem`, in any case or separator style. For example, `cliproxy_management_key`, `api_key`, and `webhookSecret` are redacted. A new field that matches this rule is redacted without a code change.
