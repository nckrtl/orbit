...
The Gateway reads the snapshot from the [Metrics role's](/reference/metrics) own Prometheus, which already scrapes a node exporter on every eligible Node every 15 seconds: one SSH round trip to the Metrics Node reads four Prometheus queries covering the whole fleet, and this command replays the entry for the requested Node. No software runs on the Node itself. The Gateway answers `node.metrics_unavailable` (HTTP 422) for a Node that is not active or has no WireGuard address, and `node.metrics_unreachable` (HTTP 502) when Metrics is not enabled, the Metrics Node could not be read, or Prometheus has no samples yet for this Node.
...
[`orbit metrics:node:list`](/cli/metrics) reads the same Prometheus data for every Node the caller can access in one request, which is what `orbit top`'s dashboard uses instead of one `node:metrics` request per Node.
