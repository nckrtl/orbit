...
Human output shows a progress tree with each deploy phase as a step, revealing named deploy steps as their phase starts. Glyphs and color show waiting, running, success, and failure states, and active indicators alternate while work is in progress. Standard output and standard error from application commands appear labeled and escaped while their step runs. On failure, the tree shows the failed step's name and error, and the final lines add the error code, the selected release when available, and the request ID. `--json` writes the same events as NDJSON, one event per line, without prompts or prose.
...
| The final event is a failed result. | Nonzero. The command names the failed step and its error. |
