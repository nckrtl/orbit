---
title: "process"
description: "Install, start, stop, inspect, and remove systemd services and Docker containers owned by an AppInstance or a Node, and record App-owned process definitions."
---

A Process is Gateway intent for one systemd service or Docker container. It belongs to exactly one owner: an AppInstance, a managed Node, or, as a reusable definition, an App. The `process` family installs the runtime artifact, changes its desired state, reads a bounded log tail, and removes it.

The [App process and Schedule definitions](/reference/app-processes-and-schedules) reference owns the runtime contract, the owner rules, and the App-definition copy model. The [hibernation reference](/reference/app-dev-runtime-hibernation) owns idle halt and wake for development AppInstance Processes.
...
| --- | --- | --- |
| `--instance=ID` | AppInstance | The Gateway derives the host Node and runtime user from the AppInstance. |
...
| `--app=APP --for=ENV[,ENV]` | App definition | The Gateway records a reusable definition for `development`, `production`, or both. Production preparation copies it into a new AppInstance Process. |
...
| [`process:list`](#orbit-processlist) | List the Processes of one AppInstance or Node, or the definitions of one App. |
...
Every command accepts `--json`. Human and JSON output include the Gateway request ID.
...
```bash
orbit process:create <name> (--instance=ID | --node=ID-or-name | --app=APP --for=ENV[,ENV]) [options]
```
...
| --- | --- | --- |
| `--instance=ID` | none | Positive AppInstance ID. |
...
Install a Vite development server on an AppInstance and start it:
...
A development systemd Process runs as the Node's managed runtime user, reads the `.env` file in the recorded checkout, and receives `VITE_DEV_SERVER_CERT`, `VITE_DEV_SERVER_KEY`, and the `ORBIT_DEV_SERVER_*` values for the AppInstance Route hostname. A production Process runs as the dedicated production user from the `current` release. A Node Process receives no AppInstance environment.
...
List the Process records of one AppInstance or Node with their desired and observed states and `keep_alive`, or the definitions of one App without their commands.
...
Replace one App process definition with a complete specification. The command accepts the same runtime options as `process:create` and requires `--app` and `--for`.
...
Changing a definition changes only App-owned configuration. It makes no remote call and does not change an existing AppInstance Process copy or its runtime state.
...
Starting an AppInstance Process requires an active AppInstance on a reachable active Node. Starting a Node Process requires a reachable active managed Node. A competitor for the same AppInstance waits for at most 30 seconds and then receives `process.operation_busy`; a competitor for the same Process receives `process.runtime_lock_failed`.
...
```bash
orbit process:destroy <process> [--app=APP] [--json]
```
...
```bash
orbit process:destroy 41
orbit process:destroy queue --app=1
```

<Warning>
The command sends the request without a confirmation prompt. Systemd units are named `orbit-process-{id}-{name}.service` and containers `orbit-process-{id}-{name}`; the Gateway checks that exact owner marker before it stops or removes anything and never adopts or deletes a colliding unit or container.
</Warning>

Destroying a definition leaves every existing AppInstance copy in place.
...
- [`database`](/cli/database) connections can align with a same-Node Docker Process on a published port.
