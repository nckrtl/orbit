...
The `dns` family changes resolver configuration on the operator machine only. It sends nothing to the Gateway. Use it when a Mac needs to reach `*.test` hostnames on a development Node directly instead of through Orbit VPN DNS.
...
Route every hostname under one development TLD to an IP address, or remove that override with `--reset`.
...
| `target` | for configuration | IPv4 or IPv6 address that wildcard hostnames under the TLD resolve to. Not accepted with `--reset`. |
...
- [`node`](/cli/node) sets a Node TLD with `node:add --tld`, which is the TLD that generated Route hostnames use.
- [`route`](/cli/route) lists the hostnames that resolve under a TLD.
