...
<Note>
Disabling `herdr` does not stop, remove, or change anything the extension created on a Node. A Herdr session keeps running after you disable the `herdr` extension; use `herdr:session:destroy` first if you want it gone.
...
- [`herdr`](/cli/herdr) and [`proxycli`](/cli/proxycli) are the extension families.
