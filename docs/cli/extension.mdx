...
The CLI stores the enabled set in `$ORBIT_HOME/extensions.json`, next to the Gateway profiles in `config.json`. The file is private to your user; the CLI refuses to read or write it when it is a symlink, is not a regular file, or is readable by other users. The CLI ignores an extension slug it does not know, such as one from an older release, and drops it the next time it writes the file.
...
| --- | --- |
| `proxycli` | The [`proxycli`](/cli/proxycli) family: fleet CLIProxyAPI quota collection. |
...
| --- | --- | --- |
| `extension` | yes | Extension slug, for example `proxycli`. |
...
Disable one extension. Its commands disappear from `orbit list`, and running one directly fails with `extension.disabled`. The command changes only local state. It never calls the Gateway.
...
<Note>
Disabling `proxycli` only hides the `proxycli:*` commands on this machine. The collector keeps running for the fleet. To stop the fleet feature, run [`proxycli:disable`](/cli/proxycli#orbit-proxyclidisable) while the extension is enabled.
</Note>

## Related

- [`proxycli`](/cli/proxycli) is the extension family.
