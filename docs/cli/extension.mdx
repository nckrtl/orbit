...
Disabling `proxycli` also disables the fleet feature when the CLI can reach the Gateway: the collector Process stops, `proxycli.orbit` is withdrawn, and the web quota UI hides. Use `proxycli:disable` when you want that fleet stop without hiding the local commands.
</Note>
