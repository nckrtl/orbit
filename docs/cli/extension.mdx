...
Human output is one table with the columns Extension and State. JSON output returns an `extensions` array with `extension` and `enabled` for each entry.
...
An unknown slug fails with `extension.unknown`. A configuration file that is malformed or not private fails with `extension.config_invalid` and leaves the file unchanged.
