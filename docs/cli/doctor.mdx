...
Human output shows a progress indicator while the request is in flight, then a table with one row per Node and family, and one additional row per finding. The table lists Node name, family, status, checked count, and any findings. Empty results state that no matching records were found. The last lines state whether the fleet is healthy and the request ID. JSON output returns the same report as one document.
