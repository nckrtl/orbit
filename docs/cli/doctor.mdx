...
For a development Instance, `instance.source_identity_mismatch` means the checkout no longer holds the recorded history: the recorded starting commit is missing, or `HEAD` shares no history with it. A checkout on another branch, on a detached `HEAD`, or reset below the starting commit is normal development work, not drift. Commands that act on the recorded branch check it when they run. For example, `instance:remove` refuses a checkout on another branch with `instance.source_branch_mismatch`.
