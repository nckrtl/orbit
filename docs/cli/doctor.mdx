...
| `--family=FAMILY` | every family | Limit checks to one family. Repeat the option for more than one. Accepted values are `node`, `role`, `app`, `instance`, `workspace`, `schedule`, `tool`, `process`, `firewall`, `herdr`, and `database_connection`. |
...
Families run in canonical order whatever filter you pass: `node`, `role`, `app`, `instance`, `workspace`, `schedule`, `tool`, `process`, `firewall`, `herdr`, `database_connection`. Nodes sort by name and then ID, and resources sort by ID, so two runs against the same state produce the same report.
...
| `app` | The repository origin that AppInstances on the Node project for the App. | `app.repository_origin_mismatch`, `app.node_unreachable`, `app.inspection_failed` |
...
| `process` | Desired state against the observed systemd or Docker status of each AppInstance and Node Process. | `process.runtime_missing`, `process.state_mismatch`, `process.node_unreachable`, `process.inspection_failed` |
