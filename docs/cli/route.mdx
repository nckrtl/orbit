...
A Route is an App-owned domain with one routing scope: a Node or an active Cluster. Development Routes have one target. Production Routes can have an ordered target pool. Orbit generates a Route for every App instance it creates. Generated domains use an explicit hostname, an active Cluster TLD, then the Node TLD. The `route` family manages explicit Routes and their targets and domains.
