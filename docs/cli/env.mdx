...
The Gateway accepts an active Instance only after its placement is complete and no source migration or in-flight Route replacement is pending. A finished public Route may keep a terminal public-edge `replacement_step` such as `public-activated` or `ingress-firewall` when `failed_step` and `error_code` are empty. An Instance without a recorded source profile returns `instance.source_profile_missing`; repeat its creation command with `--recover-source-profile` to record one.
