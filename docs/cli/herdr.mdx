...
Human output shows a detail tree with every field the JSON returns: pane, terminal, scope, columns, rows, expiry, nonce, and request ID, alongside the observer URL. The observer URL stays on its own value line so it stays easy to copy. It is a short-lived credential; do not share it outside the browser that will use it.
