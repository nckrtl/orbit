...
The Gateway answers `database.sql_multiple_statements` when the statement is stacked. SQLite query runs on the associated Node and answers `database.sqlite_node_required` when that Node is missing. Query returns at most 500 rows.
