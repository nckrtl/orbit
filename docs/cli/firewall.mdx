...
`GET /api/v1/nodes/{node}/managed-firewall-rules` lists the `orbit:` rules for one Node: the ones every managed Node has, then the ones each active role needs, with the role named. They come from the same catalog that convergence applies, so the list is what Orbit intends for the Node, and [Doctor](/cli/doctor) compares it with the Node. No request adds, changes, or removes one of these rules. The Orbit web page shows them on the Node page as locked rows; the CLI has no command for them yet.
