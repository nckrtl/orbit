...
- **Database connection** — Saved MySQL, PostgreSQL, or SQLite credentials. The Gateway can create a MySQL user and database through a Node Docker Process and then register the connection. Attaching a connection populates the App instance's stored environment. See [Database connections](/reference/database-connections).
