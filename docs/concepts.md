...
- **Legacy Instance** — A leftover application row that Doctor can still inspect. Runtime publication, overlap checks, and App instance commands do not use it.
- **Workspace** — A leftover Git worktree row owned by a Legacy Instance. Orbit exposes no Workspace command. App instance creation and runtime publication do not use or change Workspace source.
