...
- **AppInstance** — One placement of an App on one Node for development or production. It has its own checkout and branch and inherits the App's web root. Its Cluster comes from its Node.
- **Route** — A hostname that sends traffic to one or more AppInstances of one App. A Route without a target answers with a fixed unavailable response.
- **Router** — The Node role that receives traffic for an active Cluster with a TLD and forwards it to the Node that runs the target AppInstance. An active Cluster without a TLD has no Router.
