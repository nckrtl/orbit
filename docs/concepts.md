...
- **Instance** — One managed copy of a Project on a Node. app-dev uses a checkout or worktree. New app-prod Instances require a candidate. Active `laravel-app` Instances have one Route. `monorepo` and `laravel-package` Instances have no Route by default. See [ADR 0047](/decisions/0047-create-production-appinstances-from-candidates), [ADR 0106](/decisions/0106-derive-instance-capabilities-from-project-type), and [Applications](/domains/applications).
