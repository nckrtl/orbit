...
- **App** — An application record with one Git repository, a default branch, and a web root. App instances share these defaults. An explicit App update reconciles those inherited defaults without replacing the App. See [Apps](/reference/apps#update-an-app).
