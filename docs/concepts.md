...
- **App** — An application managed by Orbit. It owns the Git repository,
  ...
  conversion supplies them. See [Apps](reference/apps.md) and
...
- **AppInstance** — One place on a Node where an App is developed or runs in
  production. It belongs to the App and Node; any Cluster placement and routing
  scope comes from the Node. A managed-clone development AppInstance owns an
  independent Git clone, a branch, an immutable checkout path, a starting
  commit, and an optional relative web root override. See
  [Applications](domains/applications.md) and
  [ADR 0017](decisions/0017-optional-cluster-placement-and-tld-precedence.md).
