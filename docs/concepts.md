...
- **App** — An application managed by Orbit. It owns one repository identity and access URL, plus the `default_branch` and relative web root that every AppInstance inherits. See [Apps](reference/apps.md) and [ADR 0025](decisions/0025-stabilize-the-default-appinstance-identity.md).
...
- **AppInstance** — One managed placement of an App on one Node. It owns a `checkout` or `worktree` source; `default` is reserved for the default source. An active AppInstance has one Route. See [Applications](domains/applications.md) and [ADR 0028](decisions/0028-require-one-route-per-active-appinstance.md).
