...
Create an isolated Python environment and install `scripts/requirements.txt`. Run `python -m unittest discover -s tests -v` from this skill directory after changing either script. These are recorder and analyzer tests, not proof that an Orbit command complies.

Run `scripts/capture.py --output-dir NEW_PATH --candidate SHA --label CASE -- COMMAND...`. The output directory must not already exist. Default terminal size is 100 columns by 40 rows; set `--columns` and `--rows` for the named case. Set bounded `--timeout` and `--idle-timeout` values appropriate to the operation. `--no-live` suppresses forwarding to the parent terminal for automated recorder tests; actual Solo proof keeps live forwarding.
