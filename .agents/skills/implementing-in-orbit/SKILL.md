...
2. Call `tasks-create` with the Project's `app_id`, a short title, a brief, and `plan: true`. Write the brief from the conversation: the problem, the intended behavior, decisions already made, and open questions.
...
4. Stop working on the feature in this session. The planner shapes it in T3 and moves the group to Todo when the operator agrees the plan is ready.
