...
5. **Codify.** Put behavior in product code with a test per `Acceptance` item where the proof action is a test. Preflight already wrote the documentation; correct a page only where implementation deviates from what it states, following `writing-documentation`, and treat a deviation that changes behavior as a plan finding rather than a silent edit. Without the `docs` label, change no maintained documentation. Run focused checks, `composer docs-lint`, each changed project's `composer check`, and root `bin/test`.
