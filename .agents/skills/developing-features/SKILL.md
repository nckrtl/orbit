...
6. **Prove the exact commit.** For Incus proof, write `.loop/proof/<ISSUE>.json` with one action per `Acceptance` item whose proof names Incus. Set `observed_inputs: true` when the actions support complete PHP observations on the required surfaces; verify the planner’s decision, or make it when no plan exists. Record any opt-out reason in the handoff. Follow the collection and cleanup requirements in [proof plans](../../../docs/reference/proof-plans.md); never silently disable collection after a failure. Merge current `main`, and run `bin/e2e-topology prove <ISSUE>` while discovery remains active. Every action must exit `0`. On diagnosis, inspect with `shell --proof` or `exec --proof`, continue on discovery, then `release <ISSUE> --proof` before proving again. Leave a successful proof immutable and keep the complete `.loop/` workspace through the first pull-request approval. For later commits, including main integration, follow retained-proof evaluation below before releasing successful evidence.
...
A movement of `main` requires integration and a new evidence decision, not automatic release of a successful proof. Preserve the previous reviewed head, its included-main SHA, and the complete reviewer handoff. Integrate current `origin/main` while `.loop/` remains present; after removal, rebuild from the approved workspace head as `merging-pull-requests` requires. Run the required checks on the clean combined candidate, including `bin/test`, then run `bin/e2e-topology equivalence <ISSUE> --json` before releasing the proof. A missing current-main ancestor must be repaired before this evaluation.

- `exact` or `equivalent` with `promotion_path: retained-proof`: keep the acceptance proof.
- `equivalent` with `promotion_path: candidate-convergence`: keep the acceptance proof and run `bin/e2e-topology candidate <ISSUE>`. Require successful current-candidate convergence and general verification; do not rerun feature setup or acceptance. A failed candidate attempt needs explicit diagnosis and release with `--candidate` before retry.
- `stale` or `indeterminate`: follow the report's `next_action`, resolve listed errors, release the old proof, and run complete proof on the integrated candidate.

Changed paths alone never establish equivalence. An uninstrumented proof cannot acquire observations retrospectively, and changing its plan requires a new proof. Return the immutable equivalence report and fingerprint, selected promotion path, any candidate evidence, prior review handoff, new head/base, and current-head check results. Push and verify current-head CI before review. Let `reviewing-pull-requests` determine what assessment can be reused; approval never transfers between heads.
