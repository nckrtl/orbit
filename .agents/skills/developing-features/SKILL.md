...
Implement one Linear issue in its worktree and produce a pull-request body that proves every `Acceptance` item. Own code, tests, integration, proof, commits, and publication within the selected mode. Never review or approve your own work.

This task may be invoked directly. A supplied `.loop/plan.md` is the implementation map, not a lifecycle gate.

The execution mode is `publish` by default, or `handoff` when the caller supplies it. Both modes implement, prove, commit, and push the same exact candidate. `publish` may create or update the pull request and request review. `handoff` must not invoke `gh` or create, edit, comment on, review, approve, or otherwise mutate any pull-request or GitHub surface; it returns the complete publication payload to the caller instead.
...
7. **Publish or hand off the exact candidate.** Include current `origin/main`, commit every issue change including the complete `.loop/` workspace, push the branch, and verify the local head equals the remote branch head. The proposed body opens with `Issue: <ID>`, then lists every `Acceptance` item in the issue's order, each followed by its evidence: the test, the command output, or the proof action name. It then lists every documentation page changed and why, including the planner's pages, drift fixes from the plan's Documentation section or the no-plan audit, every reported finding from either audit with its owner, and every deviation with its reason, or `Documentation: none: <why>` when the `docs` label is absent and no page changed, or `Documentation: unchanged: <why>` when the label is present and the pages already state the outcome. End with one proof line: `Proved with .loop/proof/<ISSUE>.json at <sha>` or `Automated tests only`. Do not publish or hand off until every declared proof action has exited `0`.

   In `publish` mode, create or update the pull request with that complete body and request independent review. In caller-supplied `handoff` mode, make no pull-request or GitHub mutation and return the pushed head SHA, branch and base, the complete proposed body, exact command/check/proof results, documentation and deviation report, retained-proof binding, and every limitation. After independent approval of that exact workspace head, the implementer pushes one commit that deletes `.loop/` and changes nothing else, evaluates retained-proof equivalence, and obtains the required fresh second approval of the removal head; this later removal step is never part of the first candidate handoff.
...
- Mode selection changes publication responsibility only; it never weakens the issue contract, checks, proof, exact-head binding, branch push, or evidence payload.
