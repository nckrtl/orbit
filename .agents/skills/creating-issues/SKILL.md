...
4. Write `Acceptance` as a checklist. Each item is one observable behavior with a verification venue the current machinery supports. `Proof:` names a focused test, command, or real-machine check. Read [Implementation loop](../../../docs/reference/implementation-loop.md) for the selected delivery flow and local check policy. In default discovery, real-machine checks are reproducible development observations; only explicit proof selection requires planning to materialize Incus actions in `.loop/proof/<ISSUE>.json`. Keep generic repository checks in that shared policy rather than copying full no-TIA CI requirements into each issue. When an issue needs a specific check criterion, use focused developer tests and changed-project checks, then the Builder's root `composer check` with TIA before independent review. A sentence that fits both Scope and Acceptance is written once, as a criterion. Write every bullet on one line, never hard-wrapped, and put quoted output in backticks.
