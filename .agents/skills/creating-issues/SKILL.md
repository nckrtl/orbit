...
| Real-machine verification | the `incus` label; delivery flow determines discovery-only or separate proof |
...
4. Write `Acceptance` as a checklist. Each item is one observable behavior with a verification venue the current machinery supports. `Proof:` names an affected TIA test, command, or real-machine check. Read [Implementation loop](../../../docs/reference/implementation-loop.md) for the selected delivery flow and local check policy. In default discovery, real-machine checks are reproducible development observations; only explicit proof selection requires planning to materialize Incus actions in `.loop/proof/<ISSUE>.json`. Keep generic repository checks in that shared policy rather than copying full-suite requirements into each issue. When an issue needs a specific check criterion, use TIA development tests and changed-project checks, then the Builder's root `composer check` with TIA before independent review. A sentence that fits both Scope and Acceptance is written once, as a criterion. Write every bullet on one line, never hard-wrapped, and put quoted output in backticks.
5. Use the `incus` label when a criterion depends on a real OS, service manager, privilege boundary, network, certificate, filesystem ownership, or multi-node behavior. Omit it for automated-only work. Follow the implementation loop's Incus requirement table: the label requires discovery, and explicit `proof` selection adds a separate proof topology. Never infer or select proof from the label. Retained `proof:incus` wording means the same venue requirement; new issues use `incus`.
