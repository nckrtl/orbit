...
Use this skill within the current planning, development, or review role. It does not authorize a phase transition or replace independent review.
...
filled records in the current issue’s ignored, candidate-bound artifacts.
...
Use `verifying-cli-output` when correctness depends on prompts, terminal width, color, cursor movement, streaming, buffering, or liveness. A JSON test or final transcript cannot prove those behaviors. Return findings and evidence to the current role owner; do not claim broader command coverage than the inspected matrix supports.
