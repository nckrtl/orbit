...
describe('repository guidance bootstrap', function (): void {
    ...
    it('inventories every concrete transport operation and the Tool response DTOs', function (): void {
        ...
        $expectedOperationCount = 2 + $preScheduleOperationCount + count($scheduleRequests) + count($herdrRequests) + count($databaseRequests);
        $expectedRequests = [
    ...
    it('documents the 121-operation SDK surface including Database connection transport', function (): void {
        ...
        expect($publicContract)
            ->toContain('The SDK models exactly 121 concrete public Gateway API operations:')
            ->toContain(
            ...
            ->toContain(
                '- AppInstance: list, show, create, register, clone, transfer, remove, update, deploy-step create, list, update, and destroy, deploy, rollback, retained-release list, deployment-history list and show, environment import, environment update, environment synchronization, dependency inventory read, and dependency scan through the concise Instance routes.',
            )
        ...
        expect(repository_guidance_normalized_contents('README.md'))
            ->toContain(
                'The SDK exposes exactly 121 public Gateway operations.',
