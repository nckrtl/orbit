# ORB-239 follow-up development record

Current candidate: `d715c7cdee99b5e8c33e43b2465a0e7fd95d5cd8`

Current candidate tree: `bdfa8be8abf07af4312c951f667d7340cd7d76ad`

Current base: merged main `55aa962a5fd22bbc812d5b65ac98e865ab12e042`

Current pull request: `https://github.com/nckrtl/orbit/pull/288`

## Merged-main verification and follow-up

PR #287 merged the first repair as
`55aa962a5fd22bbc812d5b65ac98e865ab12e042`. Its required merged-main
verification ran `cd apps/e2e && composer test` in explicit fresh TIA mode.
The run exited 1 after 1,419 tests passed because the corrupt-guidance regression
still depended on the optional Pest text `Failed asserting`.

The current candidate changes only
`apps/e2e/tests/Feature/Configuration/ComposerConfigurationTest.php`. The
regression now binds the child-process failure to the stable expected repository
guidance contract text `repository-root docs/` instead of generic Pest
presentation wording. It still requires a nonzero exit and rejects the TIA
partial-run warning. The same file continues to assert the exact configured
`--tia --fresh` command.

Current candidate evidence:

- `cd apps/e2e && composer test`: passed in TIA mode; one affected test file,
  1,416 tests, and 7,804 assertions.
- `cd apps/e2e && composer check`: passed; Pest explicitly reported TIA mode,
  followed by Rector, Pint, and PHPStan.
- Root `composer check`: all 15 commands passed on the exact clean candidate.
  Receipt:
  `/home/nckrtl/orbit/.git/orbit-checks/d715c7cdee99b5e8c33e43b2465a0e7fd95d5cd8/review-fpcz94qr/result.json`.
- Every Pest log reports TIA. No Pest path, filter, group, suite, or `--no-tia`
  invocation was used.
- No runtime behavior, proof topology, dependency, schema, or maintained
  documentation changed.

The branch evidence does not clear the main correctness hold. After PR #288
merges, maintenance must rerun `cd apps/e2e && composer test` on main containing
the repair and require explicit TIA mode and exit 0 before ORB-72 may land.

## Historical first repair record
