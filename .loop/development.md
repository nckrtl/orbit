...
The second deployment supplied application-side timestamps. Its NDJSON output event sequence 4 decoded to `ORB222-FIRST 1789163410648943414` (`1789163410.648943414` seconds). Sequence 5 decoded to `ORB222-COMPLETE 1789163413655888923` (`1789163413.655888923` seconds). These values show when app-prod produced each marker, but the original stale Caddy configuration buffered their operator-side arrival. The corrected operator arrival evidence is recorded below.
...
## Review correction observations

The first reviewed candidate trapped SIGINT in the shared deployment base. A real blocked-socket diagnostic showed that PHP could defer this handler and that configuration and release-list commands inherited it. The corrected candidate leaves SIGINT at its operating-system default. The focused feature test starts the real CLI against a local Transport Layer Security (TLS) socket and checks prompt nonzero SIGINT termination plus server-observed connection closure both before response headers and while the CLI is blocked reading an open deployment stream. It also checks that deploy, rollback, configuration, and release-list commands register no custom signal.

The retained discovery Gateway imported `/etc/caddy/orbit-versions/d31522a0ed20bb5a/Caddyfile`, whose Gateway FastCGI fragment used 900-second timeouts and no flush interval. The operator rendered the accepted mounted `GatewayCaddyConfigRenderer` output and published it through `NativeGatewayCaddyConverger`, which preserved the certificate, PHP-FPM pool, and unrelated metrics fragment and ran Caddy validation. `/etc/caddy/Caddyfile` then resolved to `/etc/caddy/orbit-versions/9d4732922fb8f2b4/Caddyfile`; its Gateway fragment uses 4500-second read and write timeouts and `flush_interval 1ms`.

After that convergence, a complete operator-timed `orbit instance:deploy 2 --no-interaction` recorded:

```text
COMMAND_START 1789165193.405128857
OBSERVED 1789165194.946228594 Phase: Source preparation
OBSERVED 1789165196.284311692 Phase: Environment sync
OBSERVED 1789165196.815327320 Phase: Before activation [delayed-marker]
OBSERVED 1789165197.013416652 stdout: "ORB222-FIRST 1789165196998410400\n"
OBSERVED 1789165200.020318467 stdout: "ORB222-COMPLETE 17891652007123454\n"
OBSERVED 1789165200.297018742 Phase: Activation
OBSERVED 1789165200.835465795 Phase: Php refresh
OBSERVED 1789165201.055374098 Result: succeeded
OBSERVED 1789165201.061958675 Selected release: 20260911221955-4e4bc964a627f8a3
OBSERVED 1789165201.067705638 Request ID: 6a73016a-1738-45b9-a35f-69e7d411f55c
COMMAND_END 1789165201.071049122 EXIT 0
```

The operator received the first marker at `1789165197.013416652` and the completion marker at `1789165200.020318467`, a gap of about 3.007 seconds. The first marker therefore arrived at the operator before delayed completion, activation, and the terminal result.

A second operator-timed deployment sent SIGINT only after the first marker arrived:

```text
COMMAND_START 1789165209.023665195
OBSERVED 1789165210.285354676 Phase: Source preparation
OBSERVED 1789165211.336851378 Phase: Environment sync
OBSERVED 1789165211.859267494 Phase: Before activation [delayed-marker]
OBSERVED 1789165212.043201531 stdout: "ORB222-FIRST 178916521226073993\n"
SIGINT_SENT 1789165212.554849235 PID 4694
COMMAND_END 1789165212.561797004 EXIT 130
```

The CLI exited about 0.007 seconds after SIGINT. It emitted no completion marker, activation phase, result, or second request. The release list then showed interrupted release `20260911222010-534b7cda0adfd948` as retained but not selected; successful release `20260911221955-4e4bc964a627f8a3` remained selected. This confirms that disconnect cancellation stopped the pre-activation step and did not activate the interrupted release.

The operator restored `20260911214840-16264d91468c0c6d` with `instance:rollback`; request `b19b95e7-933c-4c95-9707-68a7329fd495` succeeded. The served response returned to `ORB-222 release one`. The persistent SQLite path, digest, and row remained `/home/orbit-app-2/database.sqlite`, `31a80bf978b3dd95991c86b09af6265e80912b50b7b07c919822f8dd14b18cde`, and `persistent-orb-222`. `bin/e2e-topology verify ORB-222` passed again for attempt `74010d1a0303380e0e7b505fee835236`.

The discovery topology remains retained with the explicit rollback release selected. Discovery development only; isolated acceptance proof not run

## Corrected candidate verification

- Corrected candidate: `a7b2db70b921bc7a2ef3ea87d7da2d8df31a77dc`.
- Corrected tree: `909e620dffc1213e214df1e844cce70704766e86`.
- Focused affected correction run: 19 tests passed with 97 assertions, including the two real TLS transport signal cases.
- `cd apps/cli && composer check`: passed guidance, Rector, Pint, and PHPStan with 0 errors.
- `composer docs-build`: passed and left `docs/generated/context.json` byte-identical at `5b644adb6c3a221d2690f05fa9e8b2ba21d08033194cfdfd1a856d267cf06b24`.
- `composer docs-lint`: passed with 0 issues, 0 errors, and 0 warnings.
- `/home/nckrtl/orbit/bin/plan-lint record ORB-222` and `verify ORB-222`: passed for plan SHA-256 `ef8b84eb5307bd5d15230b9a15018d89d5357f306efb9ab94d01936ca9ffa9c8`.
- Root `composer check`: passed all five project validation, project-check, and affected-TIA boundaries on the exact clean corrected candidate.
- Builder receipt: `/home/nckrtl/orbit/.git/orbit-checks/a7b2db70b921bc7a2ef3ea87d7da2d8df31a77dc/review-7f6z89f6/result.json`.
