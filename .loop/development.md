...
## Conflict-only main integration

GitHub reported an actual conflict after the initial candidate was pushed. Exact `origin/main` commit `cdf834b682fa312ab039b3ee9c760a77201ba3b1` was merged without restarting preflight or discovery. The three direct conflicts were limited to the release-layout lifecycle interface list, the AppInstance release-layout helper beside main's corrected casts PHPDoc, and the API test's release-layout binding. The resolution retained ORB-216 behavior and adopted main's Pint and Larastan form. The resulting feature diff against the main parent remained limited to the original 23 Gateway and maintained-documentation paths.

Post-integration checks:

- Focused Gateway selection: 215 passed, 1,937 assertions.
- `cd apps/gateway && composer check`: passed, including Pint and Larastan with no errors.
- `composer docs-build && composer docs-lint`: passed with zero documentation issues.
- `bin/e2e-topology verify ORB-216`: verified the retained discovery attempt.
