...
Flow: discovery
Candidate: `21a4810e291dca7e9bfce5436d5dac97c231a58e`
...
Reviewed conflict-resolution candidate: `be9a490a9d71fb5172f26f908bd3e9218776ba38`
Reviewed conflict-resolution artifact: `c8b445953763d046e7a49fadcd3073eaeb908f32`
...
2. Stopped independent copies
   ...
   Focused tests covered distinct Process and Schedule IDs, both supported Process backends, retained specifications, disabled desired timer state, and installation before the production target became active. The review correction adds a regression assertion that a definition which omits `restart_policy` produces the same `never` default as public Process admission. Discovery observation `production-definition-stopped-copies` inspected the real app-prod host: `orbit-process-4-queue.service` was exact-owned, inactive, and disabled; Docker container `orbit-process-3-container-worker` returned exact output `3|false` for its Process ownership label and running state; the Schedule service and timer were exact-owned and inactive, and the timer was disabled. `/home/orb225sel/current` did not exist. The systemd, Docker, and Schedule execution sentinels were all absent.
...
6. Candidate checks
   ...
   - After the PR review correction, Gateway `composer test:affected` selected `tests/Feature/Domain/InstantiateAppRuntimeDefinitionsTest.php` from the two changed files and passed 3,298 tests with 18,626 assertions.
   - Gateway `composer check`: passed `guidance:check` (12 tests, 267 assertions), Rector, Pint, and PHPStan before integration, after integration, and after the review correction.
   - `vendor/bin/pint --dirty --format agent`: passed after the review correction.
   - `git diff --check`: passed before all candidate commits.
   ...
   - Discovery `bin/e2e-topology sync ORB-225`: passed on corrected exact candidate `21a4810e291dca7e9bfce5436d5dac97c231a58e` after temporary fixture restoration.
   - Discovery `bin/e2e-topology verify ORB-225`: passed on retained attempt `9a01614644f6b90f6be26ee10b1606d0` for the corrected candidate.
   - Root `composer check`: passed on clean candidate `21a4810e291dca7e9bfce5436d5dac97c231a58e` across CLI, Docs, Gateway, E2E, and PHP SDK with TIA. Builder receipt: `/home/nckrtl/orbit/.git/orbit-checks/21a4810e291dca7e9bfce5436d5dac97c231a58e/review-htgu51xw/result.json`.
...
- A copied Process whose retained definition omits `restart_policy` uses the public admission default, `never`; explicit definition policies remain unchanged.
...
- Reviewed candidate `be9a490a9d71fb5172f26f908bd3e9218776ba38` defaulted an omitted definition `restart_policy` to `unless-stopped`, which did not preserve the accepted Process specification.
- Candidate `21a4810e291dca7e9bfce5436d5dac97c231a58e` changes that fallback to `never`, matching existing public Process admission, SDK, and CLI defaults.
- The existing retry test already creates a systemd definition without `restart_policy`; it now asserts the captured Process stores `never` before any operator mutation.
- No plan, issue, ADR, documentation, flow, or topology fixture changed. No newer main revision was integrated because no later landing conflict was reported.
