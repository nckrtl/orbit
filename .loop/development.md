...
Flow: discovery
Incus: not required
Candidate: `0182d94fee87dcda25f88e761a209bc12bb96dd8`
...
3. Bounded transport and no retry or replay
   - `packages/php-sdk/tests/Unit/Requests/Deployments/DeploymentTransportTest.php` covers both deploy and rollback with request-local `stream => true` and a 4,500-second timeout, inherited CA verification, disabled redirects, the 10-second connection timeout, NDJSON headers, response closure, pre-admission structured errors, and one-send/one-consumption behavior.
   - The affected TIA run passed.
...
7. Focused checks and exact-candidate gate
   - `composer test:affected` in `packages/php-sdk` passed with 465 tests and 1,885 assertions.
   ...
   - Builder gate receipt: `/home/nckrtl/orbit/.git/orbit-checks/0182d94fee87dcda25f88e761a209bc12bb96dd8/review-07jseqvd/result.json`.
