...
| 3. Only a succeeded terminal result exits zero; failed, malformed, truncated, or interrupted streams exit nonzero, preserve the last known selected release when available, close on Ctrl-C, and never resubmit. | Stream lifecycle and terminal-result state in the shared deployment command base, operating-system SIGINT termination and HTTP connection closure, SDK stream closure during ordinary control flow, and failure mapping. | `apps/cli/tests/Feature/Instances/DeploymentCommandsTest.php`, selected through `cd apps/cli && composer test:affected`, covers succeeded and failed results, failure before and after a selected release, malformed and missing terminal events, prompt SIGINT termination and connection closure before headers and during a blocked stream read, default SIGINT for non-stream commands, and an exact request count of one. |
...
6. Add failure and interruption tests, leave Ctrl-C at its operating-system default so it terminates promptly and closes the HTTP connection before headers or during a blocked stream read, return nonzero for interruption and every non-success terminal condition, and prove that no command retries, replays, or sends a second deployment request.
...
- The first candidate subscribed to SIGINT and tried to close the SDK stream in a handler. Real blocked-socket diagnostics showed that PHP could defer that handler and that non-stream commands inherited it. The corrected implementation leaves SIGINT at its default, so the operating system terminates the CLI and closes the HTTP connection promptly at every transport stage. This preserves the interruption outcome while changing the implementation mechanism.
- The retained discovery snapshot predates the accepted ORB-221 Caddy renderer. The corrected discovery action converges the Gateway web configuration from the mounted accepted code before it records operator-side arrival times. This is topology-only setup and changes no candidate boundary.
