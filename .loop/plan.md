...
- `docs/reference/php-runtime.md`: fixed shared-production drift and now states dedicated production service/socket identity, generated and local configuration ownership, service-specific master INI defaults and timestamp policy, validation and recovery, shared package coexistence, and the separate cache boundary.
...
- Dedicated production masters did not receive the complete app-prod OPcache profile. The renderer now generates a service-specific `master.ini`, the systemd unit appends its generated directory to `PHP_INI_SCAN_DIR`, convergence validates all seven effective settings before publication, and an existing dedicated runtime can acquire the new generated file in place.
- Package-only production convergence rejected CLI PCOV on a Node that also carries `app-dev`. It now allows and requires the existing CLI module for a dual-role Node, continues to reject FPM PCOV, and contains no PCOV mutation or systemd action.
- Partial-unit removal could delete the socket and generated projection after a failed stop. Cleanup now verifies that the selected service is inactive with `MainPID=0` before it deletes any owned runtime projection.
