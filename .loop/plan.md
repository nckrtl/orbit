...
- `docs/README.md`: links authors and reviewers to the standard.
...
Incus: required. Flow: proof. Use issue-owned discovery after independent preflight, then a separate proof topology on Beast. Snapshot recovery is prerequisite infrastructure owned by the existing ORB-91 work; do not compete with that worker or reuse its live resources as this issue's proof.
...
- Live Beast snapshot availability must be rechecked before acquisition; the last observed ORB-91 rebuild failed. This infrastructure prerequisite does not change the foundation contract or authorize a substitute for isolated proof.
