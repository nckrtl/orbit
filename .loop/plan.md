...
- The reviewed Caddy configuration used negative low-latency flush mode to disable buffering. Discovery showed that Caddy deliberately keeps the FastCGI request alive after a client leaves in that mode. The corrected implementation uses a 1 millisecond positive flush interval and a bounded, fragmented phase-line probe instead. Output remains live, while downstream disconnect state reaches PHP before the next protected boundary starts.
