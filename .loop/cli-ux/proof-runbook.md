...
Flow: proof. Incus is required. Local and Beast-host tooling exercises are not
isolated acceptance. The supported discovery acquisition refused because there
is no promoted topology snapshot generation. ORB-91 owns its recovery. No
ORB-351 topology was acquired, and no shared topology is used as a substitute.
...
When snapshot recovery completes, acquire discovery using the supported harness
and exercise the tools inside app-dev through Solo. Run the full proof on a
