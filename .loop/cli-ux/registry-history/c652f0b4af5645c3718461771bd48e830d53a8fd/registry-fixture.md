...
The expected JSON binds 107 public commands, 20 framework/internal surfaces, 127 registration keys, aliases, visibility, argument order and option names/defaults/types/descriptions, common global options, and 175 runtime/source/lock-file hashes. Full framework signatures were added from this observation because the older inventory held only their names, descriptions and visibility. Installed dependencies must match the candidate lock files. No absolute host paths occur in the compared registry object; preparation metadata retains the actual checkout path for provenance.
