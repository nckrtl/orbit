...
try {
    ...
    if (isset($options['inspect'])) {
        ...
        if (count($expectedSnapshot['public'] ?? []) !== 107 || count($expectedSnapshot['framework'] ?? []) !== 20) {
            throw new RuntimeException('Expected fixture must cover 107 public and 20 framework commands.');
        }
        $mismatches = [];
