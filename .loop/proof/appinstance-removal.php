...
set_exception_handler(static function (Throwable $exception): never {
    ...
    fwrite(STDERR, "ORB-182 fixture failed: {$errorCode}\n");
    exit(70);
});
        ...
        'members' => $removal->members
            ...
            ->map(static function (AppInstanceRemovalMember $value): array {
                ...
                return [
                    ...
                    'app_instance_exists' => AppInstance::query()->whereKey($value->app_instance_id)->exists(),
                ];
            })
            ->values()
            ->all(),
...
switch ($command) {
    ...
    case 'install-final-trigger':
        ...
        DB::unprepared(<<<'SQL'
            CREATE TRIGGER orb182_fail_final_completion
    ...
    case 'drop-final-trigger':
        ...
        DB::unprepared('DROP TRIGGER IF EXISTS orb182_fail_final_completion');
        break;

    case 'install-member-row-trigger':
        if (count($arguments) !== 1 || preg_match('/\Aorb182-[a-z0-9-]+\z/', $arguments[0]) !== 1) {
            exit(64);
        }

        DB::unprepared(sprintf(<<<'SQL'
            CREATE TRIGGER orb182_fail_member_row
            BEFORE DELETE ON app_instances
            WHEN OLD.name = '%s'
            BEGIN
                SELECT RAISE(ABORT, 'Injected member row deletion failure.');
            END
            SQL, $arguments[0]));
        break;

    case 'drop-member-row-trigger':
        if ($arguments !== []) {
            exit(64);
        }

        DB::unprepared('DROP TRIGGER IF EXISTS orb182_fail_member_row');
        break;

    case 'set-member-owner':
        if (
            count($arguments) !== 2
            || preg_match('/\Aorb182-[a-z0-9-]+\z/', $arguments[0]) !== 1
            || ! in_array($arguments[1], ['foreign', 'fixture'], true)
        ) {
            exit(64);
        }

        $app = $arguments[1] === 'fixture'
            ? fixtureApp()
            : OrbitApp::query()->firstOrCreate(
                ['slug' => 'orb182-foreign'],
                [
                    'name' => 'ORB-182 foreign owner',
                    'repository_url' => 'https://github.com/laravel/framework.git',
                    'default_branch' => '12.x',
                    'root' => 'public',
                ],
            );
        fixtureInstance($arguments[0])->update(['app_id' => $app->id]);
        break;

    default:
        fwrite(STDERR, "Unknown ORB-182 fixture command: {$command}\n");
        exit(64);
}
