...
Regression check: python3 orb352-layout.py --reject-legacy-capture FE66_TABLE_CAPTURE
...
TABLE_MINIMUM = 25
FIXTURE_GRAPHEMES = ['👩‍💻', '👍🏽', '1️⃣']
NATIVE_WIDTH_SAMPLES = {'ASCII': 5, '東京': 4, 'café': 4, '👩‍💻': 4, '👍🏽': 4, '1️⃣': 1}
...
def cell_width(text):
    """Ordinary terminal cells, without modern grapheme-width contraction.

    Combining marks and ZWJ do not advance the cursor. Each remaining codepoint
    occupies one cell, or two for East Asian Wide/Fullwidth characters. Measure
    only: never strip or split characters in the emitted content. The native
    Solo DSR samples below bound this oracle; no grapheme mode is assumed.
    """
    return sum(0 if unicodedata.category(char).startswith('M') or char == '\u200d'
               else 2 if unicodedata.east_asian_width(char) in ['W', 'F'] else 1
               for char in text)


def check_width_samples():
    actual = {text: cell_width(text) for text in NATIVE_WIDTH_SAMPLES}
    require(actual == NATIVE_WIDTH_SAMPLES, 'Width oracle differs from native DSR samples')
    return {'model': 'ordinary codepoint cells; combining marks and ZWJ have zero advance',
            'unicode_database': unicodedata.unidata_version, 'native_dsr_samples': actual,
            'modern_grapheme_mode_assumed': False}


def intact_graphemes(parts, expected):
    for grapheme in FIXTURE_GRAPHEMES:
        require(sum(part.count(grapheme) for part in parts) == expected.count(grapheme),
                'Fixture grapheme was split or lost across wrapped fragments: ' + grapheme)
...
def decode_static(raw, columns):
    ...
    append-only layout. Width uses independently counted ordinary terminal cells.
...
def fields_match(parts, expected):
    ...
    intact_graphemes(parts, expected)
...
def check_table(lines, decorated, columns):
    ...
    if columns < TABLE_MINIMUM:
        ...
        intact_graphemes(values, ''.join(expected))
    ...
    for index, line in enumerate(content):
        ...
        if not border:
            ...
            for part, width in zip(pieces[1:-1], widths):
                require(part.startswith(' ') and part.endswith(' ') and cell_width(part) == width + 2, 'Cell padding or width differs')
                row.append(part[1:-1])
...
def check_plain_block(lines, scenario, decorated):
    ...
    if scenario == 'properties':
        ...
        intact_graphemes(values[2:], ''.join(value for _, value in FIELDS))
...
def reject_legacy_capture(directory):
    """Prove the corrected oracle rejects the retained, natively shifted table."""
    check_width_samples()
    metadata = json.loads((directory / 'metadata.json').read_text())
    require(metadata['candidate'] == 'fe66e4f515c6fc161353d4cb38622630c7580d17', 'Expected the native-failure candidate recording')
    require(re.fullmatch(r'layout-table-(60x24|80x24|120x40|23x24)-(auto|plain)', metadata['label']),
            'Expected a legacy bordered-table recording')
    raw = (directory / 'raw.bin').read_bytes()
    try:
        lines = decode_static(raw, metadata['columns'])
        check_table(lines, metadata['label'].endswith('-auto'), metadata['columns'])
    except AssertionError as error:
        reason = str(error)
        require(reason.startswith('Output relies on auto-wrap:') or reason in ['Misaligned table boundary', 'Cell padding or width differs'],
                'Legacy recording failed for an unrelated reason: ' + reason)
        return {'candidate': metadata['candidate'], 'label': metadata['label'],
                'old_rendering_rejected': True, 'reason': reason,
                'raw_sha256': hashlib.sha256(raw).hexdigest()}
    raise AssertionError('The old natively shifted table incorrectly passed the corrected width oracle')
...
def main():
    ...
    width_oracle = check_width_samples()
    ...
    save(output / 'identity.json', {'candidate': candidate, 'source': str(SOURCE), 'source_identity': source_identity, 'width_oracle': width_oracle, 'php_launcher': php,
    ...
    cases += [('table', columns, 24, mode) for columns in [TABLE_MINIMUM, TABLE_MINIMUM - 1] for mode in ['auto', 'plain']]
    reports = []
    ...
    for scenario, columns, rows, mode in cases:
        ...
        try:
            ...
            expected_visible = {'detail': 'Literal markup', 'properties': 'Literal markup',
                                ...
                                'table': '│' if columns >= TABLE_MINIMUM else 'Record 2'}[scenario]
            ...
            report = {'label': label, 'passed': True, 'assertions': checked,
                      ...
                      'limitation': 'pyte 0.8.2 can drop joined-emoji suffixes or spill their cells; affected viewport rows remain identified. Raw fields, intact fixture graphemes and ordinary terminal-cell widths are asserted independently. Native Solo border/cursor inspection remains required; modern grapheme mode is not assumed.'}
...
if __name__ == '__main__':
    if len(sys.argv) == 3 and sys.argv[1] == '--reject-legacy-capture':
        print(json.dumps(reject_legacy_capture(Path(sys.argv[2])), ensure_ascii=False, indent=2))
        sys.exit(0)
    sys.exit(main())
