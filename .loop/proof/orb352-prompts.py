...
def child(arguments):
    ...
    code = 125
    try:
        process = subprocess.Popen(runtime['argv'], cwd=SOURCE, stdin=stdin)
        save(Path(runtime_path + '.started'), {'pid': process.pid})
        code = process.wait()
...
def cases():
    ...
    yield {'name': 'plain-visible-selection', 'scenario': 'select', 'mode': 'plain',
           'result': 'record-beta',
           'input': [action('Press / to search', '\x1b[B'), action('›│ 18', '\r')]}
    yield {'name': 'plain-visible-search', 'scenario': 'select', 'mode': 'plain',
           'result': 94,
           'input': [action('Press / to search', '/'), action('/ ▏', 'Gamma'),
                     action('/ Gamma▏', '\r'), action('›│ 94', '\r')]}
    yield {'name': 'plain-visible-editing', 'scenario': 'text', 'mode': 'plain',
           'result': 'accepted',
           'input': [action('Fixture name', 'accepte'), action('accepte', 'd'),
                     action('accepted', '\r')]}
    for scenario in ['text', 'password']:
        for signum in [signal.SIGINT, signal.SIGTERM]:
            yield {'name': f'{scenario}-external-{signum}', 'scenario': scenario,
                   'signal': signum, 'interrupted': signum}
        yield {'name': scenario + '-timeout', 'scenario': scenario,
               'timeout': 2, 'interrupted': signal.SIGTERM}
...
def run_case(root, candidate, spec, environment):
    ...
    expected_exit = 128 + spec['interrupted'] if 'interrupted' in spec else 20 if 'abort' in spec else 0
    ...
    capture_command = [sys.executable, str(SKILL / 'capture.py'), '--candidate', candidate,
                       ...
                       '--columns', '80', '--rows', '24', '--timeout', str(spec.get('timeout', 15)),
    ...
    if 'interrupted' in spec:
        contains.append('"interrupted":' + str(spec['interrupted']))
    elif 'abort' in spec:
    ...
    try:
        with (directory / 'capture.stdout').open('wb') as stdout, \
            ...
            capture = subprocess.Popen(capture_command, cwd=SOURCE, env=environment,
                                       stdin=subprocess.DEVNULL, stdout=stdout, stderr=stderr)
            if 'signal' in spec:
                deadline = time.monotonic() + 8
                while capture.poll() is None and time.monotonic() < deadline:
                    started = Path(str(runtime_path) + '.started')
                    raw_path = recording / 'raw.bin'
                    if started.is_file() and raw_path.is_file() and b'Fixture ' in raw_path.read_bytes():
                        pid = json.loads(started.read_text())['pid']
                        time.sleep(0.15)
                        os.kill(pid, spec['signal'])
                        checks['signal_delivered_to_waiting_prompt'] = {'pid': pid, 'signal': spec['signal']}
                        break
                    time.sleep(0.02)
                require('signal_delivered_to_waiting_prompt' in checks, 'Signal was not delivered to a visible prompt')
            code = capture.wait(timeout=22)
        checks['collector_status'] = code
        require(code == (124 if 'timeout' in spec else expected_exit), 'Collector exit differs from expected status')
        if 'timeout' in spec:
            summary = json.loads((recording / 'summary.json').read_text())
            raw = (recording / 'raw.bin').read_text()
            require(summary['child_exit_code'] == expected_exit and summary['termination'] is not None,
                    'Timeout did not preserve the interrupted child status')
            require(summary['drained'] and not summary['collector_errors'], 'Incomplete timeout capture')
            require(all(value in raw for value in contains), 'Missing timeout interruption output')
            checks['timeout_and_child_status_preserved'] = True
        else:
        ...
        if 'abort' in spec or 'interrupted' in spec:
