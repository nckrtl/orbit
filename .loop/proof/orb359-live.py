...
def stage_metrics_lifecycle():
    ...
    record(
        ...
        # Confirmed live (rehearsal #4): the real disable did not sustain the
        # required animation cadence (verify.py: "animation disappeared
        # without terminal state"). Same fix as credentials-reset-human: a
        # state_rows check that only requires the progress tree to reach its
        # settled success glyph, not a specific alternation count/interval.
        state_rows=[{
...
def stage_coverage_report():
    ...
    required_commands = {
        ...
        'metrics:exporter:disable': ['exporter-disable-human', 'exporter-disable-pipe'],
        'metrics:exporter:enable': ['exporter-enable-undecorated', 'exporter-enable-human', 'exporter-invalid-node-json'],
        'metrics:status': [
