...
if args.stage == 'prepare':
    ...
    # Restore extra to extendedAppProd: app-prod-2 must keep recipe role app-prod.
    # Prepare replaced that with app-dev. app-dev and app-prod cannot be assigned together.
    leftover = [inst for inst in read('instance:list')['app_instances'] if inst['node_id'] == extra['id']]
    assert leftover == [], ('unexpected placements on extra destination', leftover)
    ...
    leftover = [inst for inst in read('instance:list')['app_instances'] if inst['node_id'] == extra['id']]
    assert restored['status'] == 'active' and restored['roles'] == ['app-prod'] and leftover == [] and restored.get('cluster_id') is None, restored
