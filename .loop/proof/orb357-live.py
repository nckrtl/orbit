...
CANDIDATE = 'a60150b2518d060270b3586d335f9dd01e9c0c4e'
...
def record_pty(label, argv, *, expected=0, contains=None, absent=None, error_code=None, actual_gateway=True, input_actions=None, columns=100):
    out = root / label
    out.mkdir()
...
def wait_mysql(process_id: int, timeout=180) -> dict:
    import time
    deadline = time.time() + timeout
    last = None
    while time.time() < deadline:
        last = read('process:show', process_id)
        if last.get('runtime_status') == 'running':
            time.sleep(8)
            return last
        time.sleep(3)
    raise AssertionError(('mysql process not running', last))
...
if args.stage == 'prepare':
    ...
    record_pty('list-narrow', ['database:list'], contains=['ux357-sqlite'], columns=40)
    ...
    record_pty('mysql-query', ['database:query', 'ux357-mysql', 'SELECT 1 AS ok'], contains=['ok'])
    record_pty(
