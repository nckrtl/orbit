...
CANDIDATE = 'd94465d023b51edc9c5f2314b493db63ba5bf6c2'
...
if args.stage == 'prepare':
    ...
    conn.execute('CREATE TABLE IF NOT EXISTS ux357_items (name TEXT NOT NULL)')
    conn.commit()
    before = conn.execute("SELECT COUNT(*) FROM ux357_items WHERE name = 'post-state'").fetchone()[0]
    conn.close()
    record_pty(
        'query-empty-read',
    ...
    conn = sqlite3.connect(path)
    after = conn.execute("SELECT COUNT(*) FROM ux357_items WHERE name = 'post-state'").fetchone()[0]
    conn.close()
    assert after == before + 1, (before, after)
    save(root / 'sqlite-post-state.json', {'before': before, 'after': after, 'path': str(path)})
    record_pty(
        'query-insert-json',
