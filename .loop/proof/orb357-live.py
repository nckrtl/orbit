...
CANDIDATE = '42795bb1e9d6ef7479fd9a059b1d32ddeaa12c38'
...
def redacted_argv(command):
    secrets = (SENTINEL, ROOT_SECRET, USER_SECRET)
    out = []
    for part in command:
        text = str(part)
...
def record_pty(label, argv, *, expected=0, contains=None, absent=None, error_code=None, actual_gateway=True, input_actions=None, columns=100, compact_contains=None, plain=False, rows=40, animation=None):
    ...
    record = {'label': label, 'argv': redacted_argv(command), 'candidate': args.candidate, 'exit': expected, 'passed': True}
...
if args.stage == 'prepare':
    ...
    record_pty(
        ...
        contains=['Write permission', 'NULL', '1.5'],
        compact_contains=['x' * 180],
        absent=['Wrote', 'Rows affected'],
    )
    record_pty(
        'query-cells',
        ['database:query', 'ux357-sqlite', "SELECT NULL AS null_cell, '' AS empty_cell, '—' AS dash_cell, 'NULL' AS null_text, 0 AS flag, 1.5 AS amount"],
        contains=['NULL', '""', '—', '1.5'],
        absent=['Wrote', 'Rows affected'],
    )
    record_pty(
    ...
    last_frame = json.loads((root / 'query-truncated' / 'capture' / 'frames.jsonl').read_text().splitlines()[-1])
    last_text = '\n'.join(last_frame['lines'])
    assert 'Result truncated at the Gateway row limit' in last_text, last_text[-800:]
    save(root / 'query-truncated-last-frame.json', {'has_warning': True, 'final': last_frame.get('final')})
    ...
    record_pty(
        ...
        expected=1,
        contains=['ux357-sqlite', 'UX357', 'AppInstance', 'Workload .env stays unchanged'],
    ...
    expected = {
        ...
        'database:query': ['query-empty-read', 'query-insert', 'query-select-write', 'query-typed', 'query-cells', 'query-truncated', 'query-write-required', 'query-sqlite-node-required', 'query-pipe', 'mysql-query', 'mysql-grants'],
