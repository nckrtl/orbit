...
def progress_checks(waiting: str, running: str, completed: str, *, max_first_output_seconds: float = 2.0,
                     require_animation: bool = True) -> dict:
    ...
    `require_animation=False` drops the animation_rows check entirely for
    operations proven live to complete too fast for even one glyph
    alternation (`adopt`, measured on a real proof topology, attempt
    `b88d983e`: 0.51s total, 0.24s of it in the running state, one glyph
    value throughout, so `minimum_changes: 1` can never be satisfied —
    genuinely sub-second, not a flake: `docs/reference/cli-ux.md`'s
    Progress and liveness section already scopes the progress tree itself
    to work that can take longer than one second; this is that same
    threshold applied to the animation within an already-shown tree, kept
    here as a proof-methodology note rather than a change to the shared
    standard) — `state_rows` alone still proves the row reaches its
    completed label without a forbidden transition."""
...
def stage_prepare(state: Path, candidate: str) -> None:
    ...
    if not config_path.exists():
        fail('prepare', f'no gateway profile found at {config_path} after copying {source}')
        return
...
def stage_create(state: Path, candidate: str) -> None:
    ...
    # rather than mutating anything further.
    extra = None
    if not observer_unsupported:
    ...
    record(state, 'create', 'create-managed-human', candidate, args,
           decorated=True, timeout=60, idle_timeout=30, exit_code=expected_exit, contains=expected_contains,
...
def stage_observe(state: Path, candidate: str) -> None:
    ...
    record(state, 'observe', 'observe-real-pane-human', candidate, args,
           decorated=True, timeout=30, exit_code=0, contains=['Observation grant', pane])
...
def stage_adopt(state: Path, candidate: str) -> None:
    ...
    # — genuinely sub-second, not a flake. See progress_checks()'s own
    # docstring for why require_animation=False is correct here.
    ...
    record(state, 'adopt', 'adopt-external-human', candidate, args,
           decorated=True, timeout=60, exit_code=expected_exit, contains=expected_contains, extra_expectation=extra)
...
def stage_list_populated(state: Path, candidate: str) -> None:
    ...
    append_record(stage_dir(state, 'list-populated'), {
...
def stage_show(state: Path, candidate: str) -> None:
    ...
    record(state, 'show', 'show-managed-human', candidate, args,
           decorated=True, exit_code=0, contains=[MANAGED_SESSION, NODE, 'Management', 'managed'])
...
def stage_consent(state: Path, candidate: str) -> None:
    ...
    if decline_process_id is None:
        ...
        status_code, status_out, status_err = remote(ip, user, f'systemctl is-active {shlex.quote(unit)}', timeout=15)
    ...
    if create_disposable(CONSENT_HUMAN_YES_SESSION):
        ...
        record(state, 'consent', 'destroy-yes-human', candidate, yes_args,
               decorated=True, timeout=30, exit_code=0, contains=['removed from Orbit'], extra_expectation=extra)
