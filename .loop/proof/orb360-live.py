...
def progress_checks(waiting: str, running: str, completed: str, *, max_first_output_seconds: float = 2.0,
    ...
    alternation (`adopt`, measured on a real proof topology: 0.51s total,
    0.24s of it in the running state, one glyph value throughout, so
    `minimum_changes: 1` can never be satisfied — genuinely sub-second, not
    a flake; docs/reference/cli-ux.md's Progress and liveness section only
    requires a moving indicator once a step has been active for more than
    one second) — `state_rows` alone still proves the row
    reaches its completed label without a forbidden transition."""
...
def stage_adopt(state: Path, candidate: str) -> None:
    ...
    # docstring and docs/reference/cli-ux.md's Progress and liveness
    # section for why require_animation=False is correct here.
