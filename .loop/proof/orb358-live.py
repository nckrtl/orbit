...
CANDIDATE = '7255ec06c9ae4d75c811b75bf754a8ff22cdb205'
...
def record_pty(label, argv, *, expected=0, contains=None, compact_contains=None, absent=None, error_code=None, input_actions=None, columns=100, rows=40, plain=False):
    out = root / label
    out.mkdir()
    ...
    if compact_contains:
        compact = ''.join(re.sub(r'\x1b\[[0-9;?]*[A-Za-z]', '', text).split())
...
def pipe(label, argv, *, expected=0, contains=None, compact_contains=None, error=None):
    command = ['php', str(launcher), *map(str, argv)]
    print('$ ' + shlex.join(command) + ' [pipe]', flush=True)
    pipe_env = dict(env, COLUMNS='150', LINES='40')
    result = subprocess.run(command, cwd=source, env=pipe_env, input=b'', capture_output=True, timeout=240)
...
if args.stage == 'prepare':
    ...
    record_pty('process-show-definition', ['process:show', 'ux358-def', f'--app={dev["app_id"]}'], contains=['ux358-def'], compact_contains=['/bin/true'])
    record_pty(
        'process-update-definition',
        ['process:update', 'ux358-def', f'--app={dev["app_id"]}', '--for=development', '--command=/bin/false'],
        contains=['ux358-def'],
        compact_contains=['/bin/false'],
    )
    record_pty('process-list', ['process:list', f'--instance={dev["id"]}'], contains=['ux358-sleep', 'Request ID:'], columns=150)
    ...
    first = subprocess.run(
        ['php', str(launcher), 'process:create', 'ux358-assets', f'--instance={dev["id"]}', '--preset=vp-dev', '--json'],
        cwd=source, env=env, capture_output=True, input=b'', timeout=240,
    )
    first_payload = json.loads(first.stdout)
    save(root / 'preset-first.json', {'exit': first.returncode, 'payload': first_payload})
    listed = read('process:list', f'--instance={dev["id"]}')
    vp = next((p for p in listed['processes'] if p.get('name') == 'ux358-assets'), None)
    save(root / 'preset-list.json', {'vp': vp, 'names': [p.get('name') for p in listed['processes']]})
    if vp is not None:
        record_pty(
            'process-preset-create',
            ['process:list', f'--instance={dev["id"]}'],
            contains=['ux358-assets', 'failed', 'vite-environment'],
            columns=150,
        )
        exists_contains = ['vp-dev Process']
        exists_code = 'process.preset_exists'
    else:
        record_pty(
            'process-preset-create',
            ['process:create', 'ux358-assets', f'--instance={dev["id"]}', '--preset=vp-dev'],
            expected=1,
            contains=['Create Process'],
        )
        exists_contains = ['Create Process']
        exists_code = None
    record_pty(
        'process-preset-exists',
        ['process:create', 'ux358-assets-b', f'--instance={dev["id"]}', '--preset=vp-dev'],
        expected=1,
        contains=exists_contains,
    )
    record_pty(
        'process-preset-create-json',
        ['process:create', 'ux358-assets-c', f'--instance={dev["id"]}', '--preset=vp-dev', '--json'],
        expected=1,
        error_code=exists_code,
    )
    ...
    pipe('schedule-list-pipe', ['schedule:list', '--no-ansi'], contains=['ux358-tick'], compact_contains=['ux358-tick'])
    ...
    still_schedules = read('schedule:list')
    assert any(s['id'] == tick['id'] for s in still_schedules['schedules'])
    record_pty(
        'schedule-destroy-cancel',
        ['schedule:destroy', tick['id']],
        expected=1,
        contains=['cancelled'],
        input_actions=[{'wait_for': 'No', 'send': '\x03'}],
    ...
    record_pty(
        'schedule-destroy-eof',
        ['schedule:destroy', tick['id']],
        expected=1,
        contains=['cancelled'],
        input_actions=[{'wait_for': 'No', 'send': '\x04'}],
    )
    pipe('schedule-destroy-pipe', ['schedule:destroy', tick['id'], '--no-ansi'], expected=1, contains=['--yes'])
    record_pty('schedule-destroy-yes', ['schedule:destroy', tick['id'], '--yes'], contains=['ux358-tick'])

elif args.stage == 'coverage-report':
    expected = {
        'process:create': ['process-create-human', 'process-preset-create', 'process-preset-create-json', 'process-preset-invalid-option', 'process-preset-exists'],
        ...
        'schedule:destroy': ['schedule-destroy-json-no-yes', 'schedule-destroy-default-no', 'schedule-destroy-cancel', 'schedule-destroy-eof', 'schedule-destroy-pipe', 'schedule-destroy-yes'],
