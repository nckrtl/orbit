...
CANDIDATE = '9a8e0a2f96e9a99fbbf53680d783a973de6d53cb'
...
if args.stage == 'prepare':
    ...
    failed = read(
        'process:create', 'ux358-assets',
        f'--instance={dev["id"]}',
        '--preset=vp-dev',
        expected=1,
        error='vite.environment_failed',
    )
    save(root / 'preset-failed.json', failed)
    record_pty(
        'process-preset-create',
        ['process:create', 'ux358-assets-b', f'--instance={dev["id"]}', '--preset=vp-dev'],
        expected=1,
        contains=['vite-environment', 'app-dev'],
    )
    record_pty(
    ...
    expected = {
        'process:create': ['process-create-human', 'process-preset-create', 'process-preset-create-json', 'process-preset-invalid-option'],
