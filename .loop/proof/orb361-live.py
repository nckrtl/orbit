...
STAGES = [
    'doctor-baseline', 'doctor-drift-report', 'doctor-unverifiable-report', 'doctor-clean-recheck',
...
DOCTOR_ANIMATION_ROWS = [{
    'name': 'doctor-progress', 'pattern': r'(?P<glyph>[○◉●])\s+(?:Verifying|Verified) registered state',
...
if args.stage == 'doctor-baseline':
    ...
    # e2e-dev instance (instance.source_identity_mismatch, instance.inspection_failed) —
    # unrelated to this issue. The gateway Node has no App instances and is genuinely all-clear
    # in every family, so it is the honest target for the "all clear" success-path recordings.
    baseline = read('doctor')
    ...
    payload = read('doctor', '--node', str(app_dev['id']), '--family', 'instance', expected=1)
    ...
    assert 'instance.php_fpm_projection_mismatch' in codes, findings
    save(root / 'drift.json', payload)
    record(
        'drift-human', ['doctor', '--node', str(app_dev['id']), '--family', 'instance'],
        ['drift', 'instance.php_fpm_projection_mismatch', 'Healthy: no'], expected=1, columns=200,
        state_rows=DOCTOR_STATE_ROWS, animation_rows=DOCTOR_ANIMATION_ROWS,
        raw_contains=[ANSI_ORANGE], raw_absent=[ANSI_GREEN],
    )
    record(
        'drift-json', ['doctor', '--node', str(app_dev['id']), '--family', 'instance', '--json'], [],
    ...
    # the pre-existing e2e-dev instance.* findings recorded in doctor-baseline are unrelated to
    # this issue and stay exactly as they were.
    app_dev = node_with_role('app-dev')
    instance_after = read('doctor', '--node', str(app_dev['id']), '--family', 'instance', expected=1)
    instance_codes = [
        issue['code']
        for node in instance_after['nodes'] for family in node['families'] for issue in family['issues']
    ]
    assert 'instance.php_fpm_projection_mismatch' not in instance_codes, instance_codes
    ...
    save(root / 'clean-recheck.json', {'instance_after': instance_after, 'role_after': role_after})
    ...
    dev = read('instance:create', app['id'], app_dev['id'], FIXTURE_DEV_NAME)
    ...
    record(
        ...
        animation_rows=[{
            'name': 'liveness-check', 'pattern': r'(?P<glyph>[○◉●])\s+(Running liveness-check|Ran liveness-check)',
            'terminal_pattern': r'●\s+Ran liveness-check', 'minimum_changes': 2,
            'min_interval': 0.1, 'max_interval': 2.0,
        }],
        state_rows=[{
    ...
    assert home, show
    current_target = remote(prod_node, 'readlink -f ' + shlex.quote(home + '/current'))
    assert current_target.rstrip('/').endswith(rollback_result['selected_release']), (current_target, rollback_result)
    url = show.get('url')
    assert url, show
    http_status = remote(prod_node, 'curl -s -o /dev/null -w "%{http_code}" ' + shlex.quote(url))
