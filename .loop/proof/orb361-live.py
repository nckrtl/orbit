...
def pipe_interrupt_case(label, argv, *, wait_seconds, expected=130):
    """An interrupt with no PTY at all, for pipe mode and for JSON mode alike: pipe mode never
    prints anything before finish() (complete() and during() only repaint the region when
    mayRepaint is true, which requires decoration), so there is no output text a wait_for marker
    could match; JSON mode is machine output that production never runs under a real terminal,
    and record()'s capture.py always allocates a PTY, whose line discipline echoes the Ctrl-C
    keystroke into the captured bytes and corrupts NDJSON parsing (confirmed live). SIGINT is
    sent after a fixed delay instead, long enough that the silent step has started running on
    the Gateway."""
    out = root / label
    out.mkdir()
    command = ['php', str(launcher), *map(str, argv)]
...
def deploy_activity_ids(instance_id, command='instance:deploy'):
    ...
    of the window, making a plain count understate or even decrease — confirmed live. 200 is the
    API's own maximum, for comfortable margin regardless."""
    response = read('activity:list', '--limit=200')
    return sorted(
        activity['id'] for activity in response.get('activities', [])
...
if args.stage == 'doctor-baseline':
    ...
    # clear-and-redraw regression would show frames matching neither.
    prepared = load_state('deploy-prepare', 'prepared.json')
    prod_id = prepared['prod']['id']
    read(
    ...
    record(
        ...
        ['line-1', 'line-40', 'Ran chatty-step', 'Deployment succeeded.'],
        ...
        animation_rows=[{
            ...
            'minimum_changes': 1, 'min_interval': 0.02, 'max_interval': 3.0,
        }],
    )
    ...
    try:
        ...
        # 7s: liveness-check (before silent-step, --after=liveness-check) prints 5 ticks 1s
        # apart, so anything under ~6s would interrupt liveness-check itself, not the silent
        # step — confirmed live: a 1s wait produced no Gateway activity at all for the deploy,
        # consistent with interrupting so early the request may not have reached a stable phase.
        latency_pipe = pipe_interrupt_case(
            'deploy-silent-interrupt-pipe', ['instance:deploy', str(prod_id)], wait_seconds=7.0,
        )
    ...
    try:
        ...
        latency_json = pipe_interrupt_case(
            'deploy-silent-interrupt-json', ['instance:deploy', str(prod_id), '--json'], wait_seconds=7.0,
        )
        assert latency_json < 1.0, ('deploy-silent-interrupt-json', latency_json)
        stdout = (root / 'deploy-silent-interrupt-json' / 'stdout.bin').read_bytes()
    ...
    # Structural comparison only — request IDs, release names, and sequence numbers legitimately
    ...
    def ndjson_shape(raw):
        lines = [json.loads(line) for line in raw.decode().splitlines() if line]
        return {
            'types': [line.get('type') for line in lines],
            'phases': [line.get('phase') for line in lines if line.get('type') == 'phase'],
            'field_sets': [sorted(line.keys()) for line in lines],
            'last_status': lines[-1].get('status') if lines else None,
        }

    def envelope_shape(raw):
    ...
    assert envelope_shape(candidate_deploy_missing) == envelope_shape(main_deploy_missing), \
        (candidate_deploy_missing, main_deploy_missing)
    parity['deploy-missing-instance'] = envelope_shape(candidate_deploy_missing)
    ...
    assert envelope_shape(candidate_rollback_missing) == envelope_shape(main_rollback_missing), \
        (candidate_rollback_missing, main_rollback_missing)
    parity['rollback-missing-instance'] = envelope_shape(candidate_rollback_missing)
    ...
    candidate_doctor = json.loads(run_json(launcher, ['doctor'], expected=1))
    main_doctor = json.loads(run_json(main_launcher, ['doctor'], expected=1))
    assert sorted(candidate_doctor.keys()) == sorted(main_doctor.keys()), (candidate_doctor, main_doctor)
    assert sorted(candidate_doctor['summary'].keys()) == sorted(main_doctor['summary'].keys()), \
        (candidate_doctor, main_doctor)
    parity['doctor'] = sorted(candidate_doctor.keys())

    # The same fixture instance, deployed through each binary in turn: the event-type sequence,
    # phase order, and per-event field sets must match exactly.
    ...
    candidate_shape = ndjson_shape(candidate_deploy)
    main_shape = ndjson_shape(main_deploy)
    assert candidate_shape == main_shape, (candidate_shape, main_shape)
    parity['deploy'] = candidate_shape
