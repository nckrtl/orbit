#!/usr/bin/env python3
"""Forward Gateway traffic; delay a chosen GET path after recording arrival."""
import argparse
...
def main():
    ...
    parser.add_argument('--hold-path', default='/api/v1/database-connections')
    args = parser.parse_args()
    upstream = args.upstream.rstrip('/')
    hold_path = args.hold_path.rstrip('/') or '/'
    ctx = ssl.create_default_context(cafile=args.upstream_ca)
    log = []

    def persist(port):
        Path(args.state).write_text(json.dumps({'port': port, 'log': log, 'hold_path': hold_path}) + '\n')
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def forward(self):
            ...
            if self.command == 'GET' and path == hold_path:
                arrived = time.time()
